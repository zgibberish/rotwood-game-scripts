...
local FrontEnd = Class(function(self, name)
	...
	self.helptext:SetAnchors("center","bottom")

	local help_height = 80

	self.helptextbg = self.helptext:AddChild(Image("images/global/square.tex"))
	self.helptextbg:SetScale(RES_X/63 + 8, 2*help_height/63)
	self.helptextbg:SetPosition(0, -help_height/2)
	--self.helptextbg:SetClickable(false)
	self.helptextbg:SetMultColor(0,0,0,.75)

	self.helptexttext = self.helptext:AddChild(Text(FONTFACE.DEFAULT, 30))
	--self.helptexttext:SetAnchors("center","center")
	self.helptexttext:SetRegionSize(RES_X*.9, help_height)
	...
	self:SetCursor(cursor.style.pointer)
...
function FrontEnd:UpdateControls(dt)
	if not self:IsControlsDisabled() then
		--Spinner repeat
		if not (TheInput:IsControlDown(Controls.Digital.PREVVALUE) or
				TheInput:IsControlDown(Controls.Digital.NEXTVALUE)) then
			self.spinner_repeat_time = -1
		...
		--Scroll repeat
		if not (TheInput:IsControlDown(Controls.Digital.MENU_SCROLL_BACK) or
				TheInput:IsControlDown(Controls.Digital.MENU_SCROLL_FWD)) then
			self.scroll_repeat_time = -1
		...
		elseif TheInput:IsControlDown(Controls.Digital.MENU_SCROLL_BACK) then
			local repeat_time =
			TheInput:GetControlIsMouseWheel(Controls.Digital.MENU_SCROLL_BACK) and
			...
			local repeat_time =
			TheInput:GetControlIsMouseWheel(Controls.Digital.MENU_SCROLL_FWD) and
			MOUSE_SCROLL_REPEAT_TIME or
			SCROLL_REPEAT_TIME
			if self.scroll_repeat_time < 0 then
				self.scroll_repeat_time = repeat_time > dt and repeat_time - dt or 0
		...
		-- victorc: hack - local multiplayer, maybe this is not a true hack ...
		-- If the screen implements GetPlayer for player-specific customization, etc. then
		...
		if self.repeat_time > dt then
			if not inputFunction(input, Controls.Digital.MENU_LEFT) and
			   not inputFunction(input, Controls.Digital.MENU_RIGHT) and
			   not inputFunction(input, Controls.Digital.MENU_UP) and
			   not inputFunction(input, Controls.Digital.MENU_DOWN) then
				self.repeat_time = 0
			else
...
function FrontEnd:Update(dt)
	...
	self.helptext:Hide()
	--#FTF: no controller help text
	if false and
		TheInput:ControllerAttached() and
...
function FrontEnd:ShowConsoleLogInTopLeft()
	self.console_root:SetRegistration("left", "top")
	self.console_root:SetAnchors("left", "top")
	return self:ShowConsoleLog()
end
...
function FrontEnd:Fade(in_or_out, time_to_take, cb, fade_delay_time, delayovercb, fadeType)
	...
	if self:IsFading() and ((self.fadecb or self.delayovercb) or warn_about_non_cb_fades) then
...
function FrontEnd:SetCursor(c)
	if self.current_cursor ~= c then
		TheLog.ch.FrontEnd:print("SetCursor", c)
		kassert.assert_fmt(cursor.style[c], "Unknown cursor '%s'.", c)
...
function FrontEnd:OnInputEvent( fn, controls, device_type, trace, device_id )
	...
	for i = #self.screenstack, 1, -1 do
		local screen = self.screenstack[i]
		-- victorc: hack - local multiplayer, ignore non-owning player inputs if the screen has an owning player
		-- jambell: hack? - local multiplayer, ignore input from other devices if the screen has an owning device
		if (screen:GetOwningPlayer() ~= nil
			and device_type == "gamepad"
			and screen:GetOwningPlayer() ~= TheInput:GetDeviceOwner(device_type, device_id))
		or (screen:GetOwningDeviceId() ~= nil
			and device_type == "gamepad"
			and screen:GetOwningDeviceId() ~= device_id)
		then
			goto continue
		end
...
function FrontEnd:OnControlUp( controls, device_type, device_id )
	...
	--	txt = txt..(control.displayname or "").." "
	--end
