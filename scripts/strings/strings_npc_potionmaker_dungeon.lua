	...
	QUESTS =
	{
		...
		first_meeting = {
			...
			NO_RESOURCES_ALT = [[
				agent:
					...
					!angry
					<i>I barely sssmell any <#KONJUR>{name.konjur}</> on you</i>.
			]],
			...
			NO_RESOURCES_ALT2 = [[
					...
					!point
					Do come back for a <#RED>{name.potion}</> when you have some <#KONJUR>{name.konjur}</>, won't you?
			]],
			...
			POTION_FULL_ALT = [[
				...
				--mid branch--
					...
					OPT_FULL_HEALTH_RESPONSE = [[
						agent:
							!point
							Why yes, I'll consider that a legally binding verbal agreement!
					]],
				--mid branch end--
			...
			TALK2 = [[
				agent:
					...
					!shocked
					[recipe:admission_recipe] And all for only a modessst sum of <#KONJUR>{name.konjur}</>!
			]],
			...
			player_emptied_flask = {
				...
				TALK = [[
					agent:
						...
						!shocked
						[recipe:admission_recipe] This veritable panacea of a <#RED>{name.potion}</> can be <i>yours</i> for only a modessst sum of <#KONJUR>{name.konjur}</>!
				]],
			...
			second_meeting = {
				...
				OPT1_NOFUNDS = "Err, yeah, but I'm still a bit low on {name.konjur}.",
				...
				OPT1_RESPONSE = [[
					agent:
						...
						One sip with <p bind='Controls.Digital.USE_POTION' color=BTNICON_DARK>'s guaranteed to <#RED>Heal</> your woundsss, clear your skin, and leave your breath minty fresh for that <i>ssspecial someone</i> back home.
						!shocked
						[recipe:admission_recipe] This veritable panacea <#RED>Health {name.potion}</> can be <i>yours</i> for only a modessst sum of <#KONJUR>{name.konjur}</>!
				]],
		...
		seen_missing_friends = 
		{
			...
			TALK2_CAN_BUY = [[
				agent:
					!dubious
					[recipe:admission_recipe] Can't resist the call of these fine <#RED>Potions</>, can you? Only a handful of <#KONJUR>{name.konjur}</> a pop!
			]],

			TALK2_NO_RESOURCES = [[
				agent:
					!dubious
					But you still don't have enough <#KONJUR>{name.konjur}</>.
			]],
			...
			sedament_tundra = {
				TALK = [[
					agent:
						...
						!gesture
						Only <#KONJUR>10 {name.konjur}</> more than a regular <#RED>Potion</>!
				...
				OPT1B_RESPONSE_ALT_NOFUNDS = [[
					!shocked
						Why, my dear <#BLUE>{species}</>, it appears you haven't the <#KONJUR>{name.konjur}</> for this ssspecialty item!
						!dejected
						What a ssshame.
						!greet
			...
			no_resources =
			{
				TALK_NO_RESOURCES = [[
					agent:
						...
						[recipe:admission_recipe] A little <#KONJUR>{primary_ingredient_name}</> is all I ask. It's for their college fundsss. I'm sure you understand.
				]],
			...
			shop =
			{
				TALK_MINIGAME = [[
					agent:
						...
						!gesture
						[recipe:admission_recipe] Ssspare a little <#KONJUR>{primary_ingredient_name}</> for your pal {name.npc_doc_firstname} and we'll set you up with some gen-u-ine <#RED>{name_multiple.potion}</>!
				]],
