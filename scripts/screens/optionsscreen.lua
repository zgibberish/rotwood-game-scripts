...
local OptionsKeybindingScreen = Class(Screen, function(self, controlName, currentKey, callback, bind_target)
	...
	self.title = self.textContainer:AddChild(Text(FONTFACE.DEFAULT, FONTSIZE.KEYBINDING_TITLE, "", UICOLORS.LIGHT_TEXT))
		:SetText(string.format(STRINGS.UI.OPTIONSSCREEN.KEYBINDING_TITLE, self.controlName))
...
function OptionsScreen:OnChangeTab(tabButton)
	...
	if not self.currentPage:IsEmpty() then
		self.default_focus = self.currentPage.children[1]
...
function OptionsScreen:_BuildGraphicsPage()
	...
	-- We have support for multiple cursors, but we only have one.
	--~ self.pages.graphics:AddChild(OptionsScreenSpinnerRow(self.rowWidth, self.rowRightColumnWidth))
	--~ 	:SetText("Cursor")
	--~ 	:SetValues({
	--~ 		{ name = "EXTRA SMALL", desc = "The smallest cursor.", data = 1 },
	--~ 		{ name = "SMALL", desc = "The small cursor.", data = 2 },
	--~ 		{ name = "NORMAL", desc = "The default game cursor.", data = 3 },
	--~ 		{ name = "LARGE", desc = "The large cursor.", data = 4 },
	--~ 		{ name = "EXTRA LARGE", desc = "The largest cursor.", data = 5 },
	--~ 		{ name = "SYSTEM", desc = "The default system cursor.", data = 0 },
	--~ 	})
	--~ 	:HookupSetting("graphics.cursor", self)
...
function OptionsScreen:_BuildControlsPage()
	...
	local function ConfigureKeybind(bind_target, settings_key, binding_pretty_label)
	...
	local function CreateBindingPage(bind_target)
		...
		ConfigureKeybind(bind_target, "bindings.crafting",  STRINGS.UI.OPTIONSSCREEN.SETTINGS.CONTROLS.BINDINGS.CRAFTING)
		ConfigureKeybind(bind_target, "bindings.inventory", STRINGS.UI.OPTIONSSCREEN.SETTINGS.CONTROLS.BINDINGS.INVENTORY)
		ConfigureKeybind(bind_target, "bindings.interact",  STRINGS.UI.OPTIONSSCREEN.SETTINGS.CONTROLS.BINDINGS.INTERACT)
		ConfigureKeybind(bind_target, "bindings.emote",     STRINGS.UI.OPTIONSSCREEN.SETTINGS.CONTROLS.BINDINGS.EMOTE)
		...
		then
			local function CreatePseudoBinding(control, binding_pretty_label)
			...
			CreatePseudoBinding(Controls.Digital.ATTACK_LIGHT, STRINGS.UI.OPTIONSSCREEN.SETTINGS.CONTROLS.BINDINGS.LIGHT_ATTACK)
			CreatePseudoBinding(Controls.Digital.ATTACK_HEAVY, STRINGS.UI.OPTIONSSCREEN.SETTINGS.CONTROLS.BINDINGS.HEAVY_ATTACK)
		end

		ConfigureKeybind(bind_target, "bindings.light_attack", STRINGS.UI.OPTIONSSCREEN.SETTINGS.CONTROLS.BINDINGS.LIGHT_ATTACK)
		ConfigureKeybind(bind_target, "bindings.heavy_attack", STRINGS.UI.OPTIONSSCREEN.SETTINGS.CONTROLS.BINDINGS.HEAVY_ATTACK)
		ConfigureKeybind(bind_target, "bindings.dodge",        STRINGS.UI.OPTIONSSCREEN.SETTINGS.CONTROLS.BINDINGS.DODGE)
		ConfigureKeybind(bind_target, "bindings.potion",       STRINGS.UI.OPTIONSSCREEN.SETTINGS.CONTROLS.BINDINGS.POTION)
		ConfigureKeybind(bind_target, "bindings.skill",        STRINGS.UI.OPTIONSSCREEN.SETTINGS.CONTROLS.BINDINGS.SKILL)
	end
...
function OptionsScreen:_BuildOtherPage()
	...
	self.pages.other:AddChild(OptionsScreenToggleRow(self.rowWidth, self.rowRightColumnWidth))
		:SetText(STRINGS.UI.OPTIONSSCREEN.SETTINGS.OTHER.METRICS_TITLE)
		:SetValues({
			{
				name = STRINGS.UI.OPTIONSSCREEN.SETTINGS.OTHER.METRICS_DESC.ON.NAME,
				desc = STRINGS.UI.OPTIONSSCREEN.SETTINGS.OTHER.METRICS_DESC.ON.DESC,
				data = true,
			},
			{
				name = STRINGS.UI.OPTIONSSCREEN.SETTINGS.OTHER.METRICS_DESC.OFF.NAME,
				desc = STRINGS.UI.OPTIONSSCREEN.SETTINGS.OTHER.METRICS_DESC.OFF.DESC,
				data = false,
			},
		})
		:HookupSetting("other.metrics", self)
