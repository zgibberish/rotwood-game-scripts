...
local STYLES =
{
    ...
    light = {
        ...
        text = {font=FONTFACE.DEFAULT, size=28, colour=WEBCOLORS.BLACK},
    },
    dark = {
        bgconstructor = function(root, title, buttons, button_spacing, longness, body_text)
            return root:AddChild(TEMPLATES.CurlyWindow(600, longness.height, title, buttons, button_spacing, body_text))
        end,
        text = {font=FONTFACE.DEFAULT, size=28, colour=WEBCOLORS.WHITE},
    },
    dark_wide = {
        bgconstructor = function(root, title, buttons, button_spacing, longness, body_text)
...
local PopupDialogScreen = Class(Screen, function(self, title, text, buttons, spacing_override, longness, style)
    ...
    if self.dialog.body then
        self.dialog.body:SetGlyphColor(self.style.text.colour)
    end
...
function PopupDialogScreen:OnControl(controls, down)
    if PopupDialogScreen._base.OnControl(self,controls, down) then return true end

    return self.oncontrol_fn(controls, down)
end
