...
local EquipmentComparisonScreen = Class(Screen, function(self, player, new_item_def, currency_type, cost)
	...
	self.purchaseButton = self.buttons:AddChild(ActionButton())
		...
		:SetOnClick(function()
			...
			-- TODO: Pop Up "Equip or Ship" screen
...
function EquipmentComparisonScreen:PreviewWeight()
	...
	for _,slot in ipairs(relevant_slots) do
		local item = self.player.components.inventoryhoard:GetEquippedItem(slot)
		if item then
			weights[slot] = item:GetDef().weight
		else
	...
	local previewed_weight = self.itemDef.weight
	weights[self.itemDef.slot] = previewed_weight
