...
function MainScreen:DoInit()
	...
	--~ self.game_logo = self:AddChild(Image("images/ui_ftf/logo_multiplayer_stress_test.tex"))
	--~ 	:Offset(0, 398)
	--~ 	:SetScale(1.4)
	...
	self.corner_btns = self:AddChild(Widget())
		:SetName("Corner buttons")
	self.exit_button = self.corner_btns:AddChild(templates.Button(STRINGS.UI.MAINSCREEN.QUIT))
		:SetKonjur()
		:SetFlipped()
		:ResizeToFit(90)
	...
	self.options_button = self.corner_btns:AddChild(templates.Button(STRINGS.UI.PAUSEMENU.OPTIONS_BUTTON))
		:SetSecondary()
		:ResizeToFit(90)
		:SetOnClick(function() self:OnClickOptions() end)
		:SetMultColorAlpha(0)
		:Hide()
	self.multiplayer_back_button = self.corner_btns:AddChild(templates.Button(STRINGS.UI.MAINSCREEN.BACK))
		:SetSecondary()
		:ResizeToFit(90)
		:SetOnClick(function() self:OnClickMultiplayerBack() end)
		:SetMultColorAlpha(0)
		:Hide()
	self.online_back_button = self.corner_btns:AddChild(templates.Button(STRINGS.UI.MAINSCREEN.BACK))
		:SetSecondary()
		:ResizeToFit(90)
	...
	self.online_play_info_label = self.interaction_play:AddChild(Text(FONTFACE.DEFAULT, FONTSIZE.SCREEN_TEXT))
		...
		:SetHAlign(ANCHOR_MIDDLE)
		:SetAutoSize(900)
		:SetText(STRINGS.UI.MAINSCREEN.ONLINE_PLAY_INFO)
	...
	-- if not Profile:GetLastSelectedCharacterSlot() then
	-- 	self.dbg_quickplay:Disable()
	-- end
	...
	self.dbg_play_root
		...
		:Offset(-bottom_pad, bottom_pad + 60)
...
function MainScreen:LayoutCornerButtons()
	self.corner_btns:LayoutChildrenInRow(20)
	self.corner_btns:LayoutBounds("left", "bottom", self)
		:Offset(bottom_pad, bottom_pad)
	return self
end
...
function MainScreen:FocusTest_ValidateSaveFiles()
	...
	if version ~= TheSaveSystem.progress.GLOBAL_VERSION then
		local confirmation = ConfirmDialog(nil, nil, true,
			"Save Data Not Compatible",
			nil,
			"Thank you for playing!\nUnfortunately, your old data is no longer compatible.")

		confirmation:SetYesButtonText("Erase Save Data")
			:HideNoButton()
			:HideArrow()
			:CenterText()
			:CenterButtons()
			:SetOnDoneFn(function()
				...
				TheSaveSystem:EraseAll(function()
					...
					local ok_popup = ConfirmDialog(nil, nil, true,
						"Old Save Data Wiped",
						nil,
						"Your old save files have been erased.")

					ok_popup:SetYesButtonText("OK")
						:HideNoButton()
						:HideArrow()
						:CenterText()
						:CenterButtons()
...
function MainScreen:_AnimateInInteractionPlay()
	...
	self:RunUpdater(Updater.Parallel{
		Updater.Do(function() self.interaction_play:Show()
...
function MainScreen:_AnimateInInteractionMultiplayer()
	...
	-- Animate stuff in
	self:RunUpdater(Updater.Parallel{
		Updater.Do(function() self.interaction_multiplayer:Show()
			:LayoutBounds("center", "center", 0, self.interactions_container.y)
			:AnimateIn()
