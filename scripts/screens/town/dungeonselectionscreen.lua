...
function DungeonSelectionScreen:ShowLockedRegionError(invalid_players)
	if not self.lockedregion_error_widget_displayed then
		...
		self.lockedregion_error_text:SetText(error_text)
...
function DungeonSelectionScreen:PromptToTravel()
	...
	self.confirm = ConfirmDialog(nil, self.current_marker, true, "Travel to this location?")
		:SetArrowUp()

	self.confirm:SetYesButtonText("Onwards!")
		:SetNoButtonText("Wait a minute.")
...
function DungeonSelectionScreen:OnTravelToLocation(locationData)
	if locationData.type == biomes.location_type.DUNGEON then
		-- Send a 'requestrun' message to the host:
		local playerID = self:GetOwningPlayer().Network:GetPlayerID()
		RoomLoader.RequestRunWithLocationData(playerID, locationData)
		self:OnCloseButton()
	end
end
...
function DungeonSelectionScreen:SetOwningPlayer(player)
	...
	if self.sidebar then
		self.sidebar:SetOwningPlayer(player)
	end
