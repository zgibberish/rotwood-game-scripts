...
local PlayerHud = Class(Screen, function(self)
	...
	self.game_world.OnUpdate = function(game_world) 
		game_world:SortChildren(function(a, b) return a.z > b.z end) 
	...
	if TheWorld:HasTag("town") then
		...
		if PLAYTEST_MODE then
			self.feedback_text = self.root:AddChild(Text(FONTFACE.DEFAULT, 25 * HACK_FOR_4K, nil, UICOLORS.LIGHT_TEXT_TITLE))
				...
				:RightAlign()
				:SetText("Press F8 to send feedback!")
	...
	self.inst:ListenForEvent("playerentered", self._onplayerentered, TheWorld)
...
function PlayerHud:OnLowHealthStateChanged(player, is_low_health)
	...
	if is_low_health and lifetime_drinks <= 3 and player.components.potiondrinker:CanDrinkPotion() then
		self:MakePopText({ target = player, button = "Hold <p bind='Controls.Digital.USE_POTION' color=0> to Drink Potion!", color = UICOLORS.WHITE, size = FONTSIZE.BUTTON, fade_time = 5, y_offset = 150 * HACK_FOR_4K })
	end
end
...
local function dbg_start_level(roomtype)
	if TheWorld.components.propmanager:IsDirty() then
		TheFrontEnd:PushScreen(PopupDialogScreen(
				...
				"You have unsaved changes to this level. Some props were modified.",
				{
					{
						text="Cancel",
						cb = function()
...
function PlayerHud:MakeControllerSwitchPopup(playerID, device, device_id)
	...
	local title = STRINGS.UI.PRESSED_START_IN_SINGLE_PLAYER.TITLE
	if device == "keyboard" then
		title = string.format(STRINGS.UI.PRESSED_START_IN_SINGLE_PLAYER.TITLE, "<p img='images/ui_ftf/input_kbm.tex' color=0 scale=1.5>")
	elseif device_id == 1 then
		title = string.format(STRINGS.UI.PRESSED_START_IN_SINGLE_PLAYER.TITLE, "<p img='images/ui_ftf/input_1.tex' color=0 scale=1.5>")
	elseif device_id == 2 then
		title = string.format(STRINGS.UI.PRESSED_START_IN_SINGLE_PLAYER.TITLE, "<p img='images/ui_ftf/input_2.tex' color=0 scale=1.5>")
	elseif device_id == 3 then
		title = string.format(STRINGS.UI.PRESSED_START_IN_SINGLE_PLAYER.TITLE, "<p img='images/ui_ftf/input_3.tex' color=0 scale=1.5>")
	elseif device_id == 4 then
		title = string.format(STRINGS.UI.PRESSED_START_IN_SINGLE_PLAYER.TITLE, "<p img='images/ui_ftf/input_4.tex' color=0 scale=1.5>")
	end
	...
	dialog:SetOwningDeviceId(device_id)
...
function PlayerHud:OnControl(controls, down, device, trace, device_id)
	...
	if PlayerHud._base.OnControl(self, controls, down, device, device_id) then
		return true
	...
	elseif not down then
		...
		if not playerutil.AreAllPlayersDead() and controls:Has(Controls.Digital.PAUSE) then
			local player = TheInput:GetDeviceOwner(device, device_id)
			if player then
				-- keyboard uses this flow too
				-- there is no owning player in this case (free for all)
...
function PlayerHud:ShowLabelPrompt(target, player)
	...
	self.prompt = self:AddWorldWidget(FollowLabel())
	self.prompt.inst:ListenForEvent("onremove", self._onpromptremoved)
	self.prompt.inst:ListenForEvent("onremove", self._onprompttargetremoved, target)
	return self.prompt:SetTarget(target)
end
...
function PlayerHud:MakeReviveTimerText(target)
	local txt = self:AddWorldWidget(FollowRevive())
	return txt:SetTarget(target)
end
