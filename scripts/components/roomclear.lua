			...
			if enemy:IsInLimbo() then
				...
				if health and health:IsAlive() and health:GetMissing() == 0 and not enemy.HitBox:IsEnabled() then
					if enemy.components.cabbagetower then
						TheLog.ch.RoomClear:printf("[roomclearfix]: A cabbage is in limbo, and is still alive with a disabled hitbox. Pushing dying event: " .. enemy)
						TheLog.ch.RoomClear:printf("SG State: " .. enemy.sg:GetCurrentState())
						enemy:TakeControl()
						enemy:PushEvent("dying")
					else
						TheLog.ch.RoomClear:printf("[roomclearfix]: A non-cabbage is in limbo, and is still alive. Removing: " .. enemy)
						TheLog.ch.RoomClear:printf("SG State: " .. enemy.sg:GetCurrentState())
						enemy:TakeControl()
						enemy:Remove(true)
					end
