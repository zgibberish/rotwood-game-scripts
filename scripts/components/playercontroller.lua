...
local PlayerController = Class(function(self, inst)
	...
	self._onGamepadConnectedHandler = function(device_id)
		self:OnGamepadConnected(device_id)
	end

	TheInput:RegisterGamepadConnectedHandler(self, self._onGamepadConnectedHandler)
end)
...
function PlayerController:OnRemoveFromEntity()
	...
	TheInput:UnregisterGamepadConnectedHandler(self)
end
...
function PlayerController:ReleaseGamepad()
...
function PlayerController:ReInitInputs()
	...
	if self.inputID and self.inputID ~= 0 then		-- a gamepad is assigned to this Player
		local device_ent = self.input:GetDeviceOwner("gamepad", self.inputID)
		if not self.input:IsDeviceValid("gamepad", self.inputID) or (device_ent and device_ent ~= self.inst) then
			TheLog.ch.Player:printf("Unable to use pre-existing gamepad id %d (in use or invalid).", self.inputID)
			self.gamepad_id = nil
		else
			self.gamepad_id = self.input:RegisterDeviceOwner(self.inst, "gamepad", self.inputID)
			if self.gamepad_id then 
				TheLog.ch.Player:print("Registered gamepad " .. self.inputID .. " to player " .. self.inst.Network:GetPlayerID())
...
function PlayerController:SetInteractTarget(target)
	...
	if target ~= self.interacttarget then
		...
		dbassert(not new_interactable or new_interactable:CanPlayerInteract(self.inst), "Focusing a disallowed interaction.")
		local old_interactable = self.interacttarget and self.interacttarget:IsValid() and self.interacttarget.components.interactable
...
function PlayerController:OnUpdate(dt)
	...
	if not self.inst.sg:HasStateTag("busy") or self.inst.sg.statemem.canmove then
		self:_UpdateMovement()
		...
		if self.changed_cursor or self:GetLastInputDeviceType() == "keyboard" then
			...
			then
				TheFrontEnd:SetCursor(cursor.style.interact)
...
function PlayerController:_ResetCursor()
	TheFrontEnd:SetCursor(cursor.style.pointer)
	self.changed_cursor = nil
end
...
function PlayerController:OnDeviceUnregistered(device_type, device_id)
	if device_type == "gamepad" and device_id == self.gamepad_id then
		-- TheLog.ch.Player:printf("Player %s unregistered %s %s", tostring(self.inst), device_type, device_id)
		self.gamepad_id = nil
		-- victorc: hack - local multiplayer, needs to be cleaner
		self.input:RemoveGeneralControlHandlerForGamepad(device_id, self.gamepadhandler)
		self.gamepadhandler = nil
	end
end
