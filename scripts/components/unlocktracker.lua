...
function UnlockTracker:OnPostLoadWorld()
	-- TEMP FIX FOR PLAYTEST ONLY.
	-- SCAN THE PLAYER'S INVENTORY.
	-- IF THEY HAVE A WEAPON OF TYPE SHOTPUT, UNLOCK THAT WEAPON TYPE.
	if self:IsWeaponTypeUnlocked(WEAPON_TYPES.SHOTPUT) then return end

	local Equipment = require"defs/equipment"
	local weapons = self.inst.components.inventoryhoard:GetSlotItems(Equipment.Slots.WEAPON)
	for _, weapon in ipairs(weapons) do
		local weapon_def = Equipment.FindItem(weapon.id)
		if weapon_def.weapon_type == WEAPON_TYPES.SHOTPUT then
			print("TEMP: FIXING SAVE FILE, UNLOCKING SHOTPUT WEAPON TYPE")
			self:UnlockWeaponType(WEAPON_TYPES.SHOTPUT)
			break
		end
	end
end
