...
local VendingMachine = Class(function(self, inst, ware_data)
	...
	inst:ListenForEvent("on_hud_created", function() self.InitializeUi() end)
end)
...
function VendingMachine:InitializeUi()
	...
	self.root = FollowPrompt("VendingMachine")
...
function VendingMachine:SetWareData(ware_data)
	...
	self.price_tag = self:AddWidget(MakeTextWidget(self:MakePriceText(self.cost)), true)
	local purchase_text = self:IsCrowdFunded()
		and STRINGS.UI.VENDING_MACHINE.DEPOSIT
		or STRINGS.UI.VENDING_MACHINE.PURCHASE

	-- When we are crowd-fundable, the purchase "button" is just text telling what control to press to deposit. It is
	-- not an actual clickable button.
	local purchase_text_widget
	local purchase_button_is_actual_button = false
	-- TODO @chrisp #deadcode - not using actual buttons for vending machine purchases because mouse-clicks are weird
	-- in that they are just different than keypresses or gamebad button presses. Reinstate them if we get that tech
	-- worked out. If we choose to leave the inputs as non-mouse ony, maybe we leave things like this and this truly
	-- is dead code.
	-- One-time purchases work just fine with a classic button.
	-- local purchase_button_is_actual_button = not self.crowd_fundable
	if purchase_button_is_actual_button then
		self.purchase_button = self:AddWidget(Templates.Button(purchase_text):ResizeToFit(), false)
		purchase_text_widget = self.purchase_button.text
	else
		self.purchase_button = self:AddWidget(Text(FONTFACE.DEFAULT, FONTSIZE.SCREEN_SUBTITLE, purchase_text))
			:SetName("Deposit Prompt")
			:SetGlyphColor(UICOLORS.WHITE)
			purchase_text_widget = self.purchase_button
	end

	-- When the input mode changes, update the text to tell the user what button to use to deposit.
	self.purchase_button.OnInputModeChanged = function(_purchase_button, _old_device_type, _new_device_type)
		purchase_text_widget:ForceSetText(purchase_text)
	end
...
function VendingMachine:InitInteractable()
	...
	interactable
		...
		:SetOnLoseInteractFocusFn(function(_inst, player) self:OnLoseInteractFocus() end)
...
function VendingMachine:OnGainInteractFocus(player)
	...
	if self.details then
		self.details:Show()
		if self.details.SetPlayer then
...
function VendingMachine:OnLoseInteractFocus()
	if self.summary then
		self.summary:Show()
	end
	if self.details then
...
function VendingMachine:ReduceFunds(deposit, player, silent)
	...
	if not silent then
		...
		local handle = soundutil.PlaySoundData(player, params) -- TODO: audio, may not rate-limit as desired
	end
end
...
function VendingMachine:ComputeDeposit(player)
	...
	if self:IsCrowdFunded() then
		...
		local remaining_cost = math.max(0, self.cost - self:GetTotalDeposited())
		...
		deposit = math.min(DEPOSIT_RATE, remaining_cost)
...
function VendingMachine:CanDeposit(player)
	if not self.ware_id then return end
...
function VendingMachine:LocalDepositCurrency(id, delta)
	if self:IsCrowdFunded() then
		if TheNet:IsHost() then -- only handle crowdsourced items on the host
			if self:HasPlayerPurchased(id) then
...
function VendingMachine:NetDepositCurrency(id, delta)
	...
	TheNetEvent:Deposit(self.inst.Network:GetEntityID(), id, delta)
	...
	if not TheNet:IsHost() then
		...
		self._netsync_task = self.inst:DoTaskInTicks(30, function() self:_NetSyncDeposits() end)
	end
end
...
function VendingMachine:OnNetDepositCurrency(id, delta)
	...
	if self:GetTotalDeposited() > self.cost then
...
function VendingMachine:UpdatePriceTag()
	if self:IsCrowdFunded() then
		self.price_tag:SetText(self:MakePriceText(math.max(self.cost - self:GetTotalDeposited(), 0)))
	end
end
...
function VendingMachine:HasPlayerPurchased(id)
	local purchased = false

	if self:IsCrowdFunded() then
		purchased = self:GetTotalDeposited() >= self.cost
	else
		purchased = self.deposited[id] >= self.cost
	end

	return purchased
end

function VendingMachine:EmitWare(player)
	-- Run the function to generate the ware we just built, and then position it.
	local ware = self.purchased_fn(self.inst, player)
	if ware then
