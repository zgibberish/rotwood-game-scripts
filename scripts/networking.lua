...
IsLocalGame = not TheNet:IsInGame() or TheNet:IsGameTypeLocal()
...
function SpawnLocalPlayerFromSim(player_guid, playerID)
    ...
    if player ~= nil then
        ...
        player:_SetSpawnInstigator(player)
...
function LocalPlayerChangedInputID(player_guid)
    local player = Ents[player_guid]
    if player ~= nil then
		player.components.playercontroller:ReInitInputs();
    else
		print("INVALID PLAYER ENTITY! Guid: " .. player_guid)
	end
end
...
function OnNetworkClientStartRun(mode, arenaWorldPrefab, regionID, locationID, seed, altMapGenID)
	...
	if mode == STARTRUNMODE_DEFAULT then
		TheLog.ch.Networking:printf("OnNetworkClientStartRun: mode=%d regionID=%s locationID=%s motherseed=%d altMapGenID=%s",
			mode, regionID, locationID, seed, tostring(altMapGenID))

		local RoomLoader = require "roomloader"
		RoomLoader.ClientStartRun(regionID, locationID, seed, altMapGenID)
...
function OnNetworkClientLoadRoom(actionID, worldPrefab, sceneGenPrefab, roomID)
	...
	if actionID == RESET_ACTION.LOAD_DUNGEON_ROOM then
		...
		if TheWorld and TheDungeon and TheDungeon:GetDungeonMap() then
			local cardinal = TheDungeon:GetDungeonMap():GetCardinalDirectionForAdjacentRoomId(roomID)
			if cardinal then
				TheLog.ch.Networking:printf("OnNetworkClientLoadRoom: TravelCardinalDirection %s at room id %d", cardinal, roomID)
				TheDungeon:GetDungeonMap():TravelCardinalDirection(cardinal)
			end
		end
		RoomLoader.ClientLoadDungeonLevel(worldPrefab, sceneGenPrefab, roomID)
	elseif actionID == RESET_ACTION.LOAD_TOWN_ROOM then
		RoomLoader.ClientLoadTownLevel(worldPrefab, roomID)
	elseif actionID == RESET_ACTION.DEV_LOAD_ROOM then
		RoomLoader.ClientDevLoadLevel(worldPrefab)
	else
...
local function UpdateClientLoadRoom()
	...
	-- ClientDoRoomChange only returns values if this client need to change rooms. It assumes that the lua side will successfully change rooms after this call, as it internally updates the simseqnr
	local roomdata = TheNet:ClientDoRoomChange();
	if roomdata and roomdata.actionID ~= 0 then
		lastRoomCompleteAckSeqNr = 0 -- explicit reset needed when per-room sim reset goes away
		OnNetworkClientLoadRoom(roomdata.actionID, roomdata.worldPrefab, roomdata.sceneGenPrefab, roomdata.roomID)
	end
end
...
function OnPlayerEntered(playerGUID)
	...
	print("OnPlayerEntered for guid " .. playerGUID)
