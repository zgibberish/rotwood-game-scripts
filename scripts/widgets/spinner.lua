...
function Spinner:GetHelpText()
	...
	if self.leftimage.enabled then
		table.insert(t, TheInput:GetLocalizedControl(controller_id, Controls.Digital.PREVVALUE, false, false) .. " " .. STRINGS.UI.HELP.PREVVALUE)
	end

	if self.rightimage.enabled then
		table.insert(t, TheInput:GetLocalizedControl(controller_id, Controls.Digital.NEXTVALUE, false, false) .. " " .. STRINGS.UI.HELP.NEXTVALUE)
	end
...
function Spinner:AddControllerHints()
	if TheInput:ControllerAttached() then 
		...
		self.left_hint:SetString(TheInput:GetLocalizedControl(TheInput:GetControllerID(), Controls.Digital.PREVVALUE))
		self.left_hint:SetPosition( -self.width/2 + w/2 + 32, 0, 0 )

		self.right_hint = self:AddChild( Text( FONTFACE.BODYTEXT, 26 ) )
		self.right_hint:SetString(TheInput:GetLocalizedControl(TheInput:GetControllerID(), Controls.Digital.NEXTVALUE))
