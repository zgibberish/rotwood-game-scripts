...
function TextEdit:GetHelpText()
	...
	if self.editing then
		if self.cancel_helptext ~= "" then
			table.insert(t, TheInput:GetLocalizedControl(controller_id, Controls.Digital.CANCEL) .. " " .. self.cancel_helptext)
		end

		if self.apply_helptext ~= "" then
			table.insert(t, TheInput:GetLocalizedControl(controller_id, Controls.Digital.ACCEPT, false, false ) .. " " .. self.apply_helptext)
		end
	else
		if self.edit_helptext ~= "" then
			table.insert(t, TheInput:GetLocalizedControl(controller_id, Controls.Digital.ACCEPT, false, false ) .. " " .. self.edit_helptext)
		end
	end
