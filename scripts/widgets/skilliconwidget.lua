...
function SkillIconWidget:BuildTextureName(skill)
	local rarity = self.skill:GetRarity()
	local category = self.def.power_category
	local name = string.format("images/ui_ftf_powers/%s_skill.tex", rarity)
...
function SkillIconWidget:SetSkill(skill)
	...
	self.border:SetTexture( self:BuildTextureName(self.skill) )
