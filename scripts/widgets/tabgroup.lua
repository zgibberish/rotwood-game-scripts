...
function TabGroup:RefreshHotkeyIcon()
	if TheFrontEnd:IsRelativeNavigation() then
		local prev_key = Controls.Digital.MENU_TAB_PREV.key
		local next_key = Controls.Digital.MENU_TAB_NEXT.key
		
		if self.is_subtab then
			prev_key = Controls.Digital.MENU_SUB_TAB_PREV.key
			next_key = Controls.Digital.MENU_SUB_TAB_NEXT.key
		end

		self.prev_icon:SetTexture(TheInput:GetTexForControlName(prev_key))
			:Show()
		self.next_icon:SetTexture(TheInput:GetTexForControlName(next_key))
			:Show()
	else
