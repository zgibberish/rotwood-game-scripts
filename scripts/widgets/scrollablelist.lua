...
local SCROLLBAR_STYLE = {
    BLACK = {
        up = "images/ui_dst/arrow_scrollbar_up.tex",
        down = "images/ui_dst/arrow_scrollbar_down.tex",
        bar = "images/ui_dst/scrollbarline.tex",
        handle = "images/ui_dst/scrollbarbox.tex",
        --~ scale = 0.4,
        scale = 1.0,
    },
    GOLD = {
        up = "images/ui_dst/scrollbar_arrow_up.tex",
        down = "images/ui_dst/scrollbar_arrow_down.tex",
        bar = "images/ui_dst/scrollbar_bar.tex",
        handle = "images/ui_dst/scrollbar_handle.tex",
        scale = 0.3,
    }
}
...
local ScrollableList = Class(Widget, function(self, items, listwidth, listheight, itemheight, itempadding, updatefn, widgetstoupdate, widgetXOffset, always_show_static, starting_offset, yInit, bar_width_scale_factor, bar_height_scale_factor, scrollbar_style)
    ...
    self.bg = self:AddChild(Image("images/ui_dst/blank.tex")) -- so that we have focus whenever the mouse is over this thing
    ...
    -- self.widget_bg = self:AddChild(Image("images/ui_dst.xml", "1percent_clickbox.tex"))
    -- self.widget_bg:SetMultColor(1,1,1,0)
    -- self.widget_bg:ScaleToSize(self.width, self.height)

    self.scroll_bar_container = self:AddChild(Widget("scroll-bar-container"))

    self.up_button = self.scroll_bar_container:AddChild(ImageButton(self.scrollbar_style.up))
    ...
    self.down_button = self.scroll_bar_container:AddChild(ImageButton(self.scrollbar_style.down))
    ...
    self.scroll_bar_line = self.scroll_bar_container:AddChild(Image(self.scrollbar_style.bar))
    ...
    self.scroll_bar = self.scroll_bar_container:AddChild(ImageButton("images/ui_dst/1percent_clickbox.tex", "images/ui_dst/1percent_clickbox.tex", "images/ui_dst/1percent_clickbox.tex", nil, nil, {1,1}, {0,0}))
    ...
    self.position_marker = self.scroll_bar_container:AddChild(ImageButton(self.scrollbar_style.handle))
...
function ScrollableList:DebugDraw_AddSection(ui, panel)
    ScrollableList._base.DebugDraw_AddSection(self, ui, panel)

    ui:Spacing()
    ui:Text("ScrollableList")
    ui:Indent() do
        local step, step_fast = 1, 5
        local has_modified_x, out_x = ui:InputFloat("width", self.width, step, step_fast)
        if has_modified_x then
            self.width = out_x
        end
        local has_modified_y, out_y = ui:InputFloat("height", self.height, step, step_fast)
        if has_modified_y then
            self.height = out_y
        end
        local has_modified_height, out_height = ui:InputFloat("item_height", self.item_height, step, step_fast)
        if has_modified_height then
            self.item_height = out_height
        end
        local has_modified_padding, out_padding = ui:InputFloat("item_padding", self.item_padding, step, step_fast)
        if has_modified_padding then
            self.item_padding = out_padding
        end
        local has_modified_offset, out_offset = ui:InputFloat("x_offset", self.x_offset, step, step_fast)
        if has_modified_offset then
            self.x_offset = out_offset
        end
        local has_modified_initialy, out_initialy = ui:InputFloat("yInitial", self.yInitial, step, step_fast)
        if has_modified_initialy then
            self.yInitial = out_initialy
        end
        local has_modified_show, out_show = ui:Checkbox("always_show_static_widgets", self.always_show_static_widgets)
        if has_modified_show then
            self.always_show_static_widgets = out_show
        end
        ui:Value("focused_index", self.focused_index)
        ui:Checkbox("focus_children", self.focus_children)
        ui:Value("#items", #self.items)
        --~ local item_names = {}
        --~ for i,val in ipairs(self.items) do
        --~     table.insert(item_names, tostring(val))
        --~ end
        --~ ui:ListBox("items", item_names)
...
function ScrollableList:Scroll(amt, movemarker)
    ...
    -- Move the marker
    if movemarker then
        local markerx, markery = self.position_marker:GetPosition()
        ...
        self.position_marker:SetPosition(markerx, newY)
    end
            ...
            if not skipFixUp then
                ...
                v.GetHelpText = function()
                    ...
                    if self.scroll_bar and self.scroll_bar:IsVisible() then
                        table.insert(t, TheInput:GetLocalizedControl(controller_id, Controls.Digital.MENU_SCROLL_BACK, false, false).."/"..TheInput:GetLocalizedControl(controller_id, Controls.Digital.MENU_SCROLL_FWD, false, false).. " " .. STRINGS.UI.HELP.SCROLL)   
                    end
                    if helptextFn then 
...
function ScrollableList:GetNearestStep()
    local markerx, markery = self.position_marker:GetPosition()
    return math.floor((markery / self.step_size) + 0.5)
end
