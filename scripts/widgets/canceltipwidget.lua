...
function CancelTipWidget:KeepAlive( auto_increment )
	...
	if self.initialized == false then
		...
		if controller_id ==  0 then
			local_cancel_tip_widget:SetText(STRINGS.UI.NOTIFICATION.PRESS_KB..TheInput:GetLocalizedControl(controller_id, Controls.Digital.CANCEL)..STRINGS.UI.NOTIFICATION.DISCONNECT_KB)
		else
			local_cancel_tip_widget:SetText(STRINGS.UI.NOTIFICATION.PRESS_CONTROLLER..TheInput:GetLocalizedControl(controller_id, Controls.Digital.CANCEL)..STRINGS.UI.NOTIFICATION.DISCONNECT_CONTROLLER)
		end
