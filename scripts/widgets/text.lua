...
function Text:SetText(text)
	...
	if text ~= self.text then
		...
		self.formatted_text = SetFormattedText( self.inst.TextWidget, text )
...
function Text:RefreshText()
	...
	-- refreshed.
	-- TODO: can/should this be done within TextWidget? We don't want
	-- flickering, but we don't want stale text either.
...
local default_personality = {
	...
	separator = {
		-- Given one of these separators, how long should we delay before laying
		-- out the next string segment?
		["!"] = {
			delay = 0.5,
		},
		["?"] = {
