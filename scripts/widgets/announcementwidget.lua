...
local AnnouncementWidget = Class(Widget, function(self, font, size, colour)
    ...
    self.text = self.root:AddChild(Text(font or FONTFACE.DEFAULT, size or 30))
...
function AnnouncementWidget:SetTextColour(r,g,b,a)
    ...
    self.text:SetGlyphColour(colour)
end
...
function AnnouncementWidget:SetAlpha(alpha)
    ...
    self.text:SetGlyphColour(current_colour)
    
    self.colour = current_colour

    self.icon:SetMultColor(1, 1, 1, alpha)
    self.icon.bg:SetMultColor(1, 1, 1, alpha)
end
