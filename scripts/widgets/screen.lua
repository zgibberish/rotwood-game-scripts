...
-- assign the owning player for this screen if it is player-specific (i.e. inventory)
-- in practice, the player-specific input component will be used instead of TheInput singleton
-- see FrontEnd:Update
function Screen:SetOwningPlayer(owningplayer)
	-- TheLog.ch.FrontEnd:print("Screen:SetOwningPlayer=" .. tostring(owningplayer))
	assert(owningplayer == nil or EntityScript.is_instance(owningplayer))
...
function Screen:SetOwningDeviceId(device_id)
	assert(self.owningplayer == nil)
	self.owningdeviceid = device_id
end

function Screen:GetOwningDeviceId()
	return self.owningdeviceid
end
