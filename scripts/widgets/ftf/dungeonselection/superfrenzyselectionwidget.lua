...
local SuperFrenzySelectionWidget = Class(Widget, function(self, player)
	...
	if player then self:SetOwningPlayer(player) end
...
function SuperFrenzySelectionWidget:SetOwningPlayer(player)
	self.owningplayer = player
	self:OnSetOwningPlayer()
	return self
end

function SuperFrenzySelectionWidget:GetOwningPlayer()
	return self.owningplayer
end

function SuperFrenzySelectionWidget:OnSetOwningPlayer()
...
function SuperFrenzySelectionWidget:SetLocation(data)
	...
	local unlocktracker = self.owningplayer.components.unlocktracker
	local equipped_weapon_type = self.owningplayer.components.inventory:GetEquippedWeaponType()
	...
	if max_seen_level > max_allowed_level_for_party then
		...
		if limiting_player == self.owningplayer then
