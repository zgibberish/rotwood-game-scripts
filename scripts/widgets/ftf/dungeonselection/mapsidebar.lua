...
function MapSidebar:SetOwningPlayer(player)
	self.owningplayer = player

	self.dungeon_level:SetPlayer(self.owningplayer)
	self.ascension_widget:SetOwningPlayer(self.owningplayer)

	return self
end

function MapSidebar:GetOwningPlayer()
...
function MapSidebar:RefreshContents()
	...
	if self:GetOwningPlayer() then
		-- when the screen refreshes without calling set owning player again
		-- happens when closing the 'change weapon' overlay screen
		self.ascension_widget:SetOwningPlayer(self:GetOwningPlayer())
	end
