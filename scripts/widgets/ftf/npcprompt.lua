...
----
local NpcPrompt = Class(FollowPrompt, function(self, player, npc)
...
-- TODO: move this to npc tuning so each villager can override these values.
local personality = {
...
function NpcPrompt:ShowDialogBalloonSpooled(line, onClickFn, hasActionBar)
	self.balloon = self.speechBlock:AddChild(SpeechBalloon())
		:SetGainFocusSound(nil)
		:SetHoverSound(nil)
...
function NpcPrompt:ShowRecipeMenu(player, line, recipe, onClickFn)
	assert(line, "Looks weird without some text here.")
	self:ShowDialogBalloonSpooled(line, onClickFn)
