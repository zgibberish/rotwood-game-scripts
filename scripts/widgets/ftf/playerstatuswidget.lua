...
local PlayerStatusWidget = Class(Widget, function(self, owner)
	...
	self.potion_widget = self.root:AddChild(PotionWidget(60 * HACK_FOR_4K, self.owner))
...
function PlayerStatusWidget:_GetAnimOffscreenY()
	...
	h = h + 100 -- bg doesn't extend to bottom.
...
function PlayerStatusWidget:_Layout(anchors, to_middle)
	...
	self.potion_widget
		...
		:Offset(6 * to_edge.x, -60 * to_edge.y)

	self.konjur
		:LayoutBounds(anchors.after, anchors.center, self.potion_widget)

	self.meta_currency
		:LayoutBounds(anchors.left, anchors.below, self.potion_widget)
