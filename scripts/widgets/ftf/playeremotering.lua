...
function PlayerEmoteQuadrant:SetHotkeyControl(control)
	if not self.hotkey_container then
		...
		self.hotkey_bg = self.hotkey_container:AddChild(Image("images/ui_ftf_hud/circle.tex"))
			...
			:SetMultColor(self.edge_normal)
		self.hotkey_icon = self.hotkey_container:AddChild(Image("images/ui_ftf_hud/circle.tex"))
			:SetName("Hotkey icon")
			:SetSize(hotkey_size - hotkey_padding*2, hotkey_size - hotkey_padding*2)
			:SetMultColor(UICOLORS.BACKGROUND_MID)
	end

	self.hotkey_icon:SetTexture(TheInput:GetTexForControlName(control.key, "gamepad"))
...
function PlayerEmoteRing:FadeIn()
	...
	if SOUND_EVENTS.OPEN_POWERS and SOUND_EVENTS.OPEN_DEFAULT then
		self:PlaySpatialSound((self.slots_count > 0) and SOUND_EVENTS.OPEN_POWERS or SOUND_EVENTS.OPEN_DEFAULT, { Count = self.slots_count })
	end
	self:StartUpdating()
end
