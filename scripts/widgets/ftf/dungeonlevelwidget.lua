...
function DungeonLevelWidget:ShowMetaProgression(on_progress_fn)
	...
	for i, level_data in ipairs(levels_gained) do
		...
		if level_data.did_level then
			local current_reward = self.meta_reward_def.rewards[level_num]
		...
		if levels_left_to_animate == 0 then
			...
			if self.meta_reward_def.rewards[next_level] then
				local upcoming_reward = self.meta_reward_def.rewards[next_level]
				...
				table.insert(presentation, Updater.Do(function()
					...
					on_progress_fn(next_level, nil, nil, upcoming_reward, nil)
				end))
...
function DungeonLevelWidget:SetProgressData(current, max)
	...
	if xp_parameter_for_sound == 1 then
		if self.levels_gained_this_session == 1 then
			self:PlaySpatialSound(fmodtable.Event.Mus_levelUp_Stinger, { faction = self.faction, faction_player_id = self.player_id })
		end
	...
	if self.badge_radial.is_initial_draw then
