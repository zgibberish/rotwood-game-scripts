...
function DebugQuestManager:RenderPanel( ui, panel )
	...
	if ui:CollapsingHeader("Spawn") then
		...
		local filter_txt = ui:_InputText( "Filter", self.filter_box_string or "" )

		if filter_txt and filter_txt ~= self.filter_box_string then
			self:RefreshSpawnList( filter_txt )
			self.filter_box_string = filter_txt
			self:SaveQuestSettings()
		end
...
function DebugQuestManager:RefreshSpawnList( filter_txt )
	table.clear( self.spawn_quests )
	for _, qdef in ipairs( self.all_quest_defs ) do
		if filter_txt == nil or qdef:GetContentID():upper():find(filter_txt:upper(), 1, true) then
...
function DebugQuestManager:LoadQuestSettings( dbg )
	--~ local data = dbg:GetDebugSettings():GetSetting( "debug_quest_manager" )
	--~ self.filter_box_string = data and data.filter_box_string or ""
	-- HACK(dbriscoe): implement saving
	self.filter_box_string = ""
	self:RefreshSpawnList( self.filter_box_string )
end

function DebugQuestManager:SaveQuestSettings()
	--~ local save_data = {}
	--~ save_data.filter_box_string = self.filter_box_string
	--~ dbg:GetDebugSettings():ApplySetting( "debug_quest_manager", save_data )
	--~ dbg:GetDebugSettings():SaveSettings()
end
...
function DebugQuestManager:_RenderConvoTester(ui, panel)
	if self.npc_talker then
		...
		local quests = contentutil.GetContentDB():GetFiltered(Quest, function(quest_class)
			...
			local q = quest_class()
			return q:MatchesCastFilters("giver", npc_node)
		end)
