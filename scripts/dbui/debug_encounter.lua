...
function DebugEncounter:OnHealthGrowthChanged(value)
	self.static.data.growth_values[self.test_options.selected_difficulty_index] = math.floor(value)
	self.static.dirty = true
end
...
function DebugEncounter:RenderPanel( ui, panel )
	...
	local biome_names = lume(biomes.locations):keys():sort():result()
	...
	for i, encounter in iterator.sorted_pairs(selected_encounters) do
		...
		if encounter then
			...
			if ui:CollapsingHeader(string.format("[%s%%] %s encounter %s", lume.round(amount_out_of_range * 100), difficulty_name, i), i == self.view_encounter and ui.TreeNodeFlags.DefaultOpen or nil) then
