...
local berna_strings = require("strings.strings_npc_armorsmith").QUESTS.megatreemon
...
--
--
-- Quests don't spawn automatically. There are some spawned for new games in
-- contentloader.lua and otherwise, you can spawn them from quests:
...
-- TODO(dbriscoe): Changed some functions to use String when they accept pretty
-- text so writers can search for String and find those functions and the
-- Strings collection.
-- Title, Desc, Log
--
-- Renamed Dialog -> Talk for clarity (it doesn't show a pop up dialog).
-- Added AllocateEnemy
-- Added PromptString so we can quest-specific text pop up to indicate interesting dialogue.
-- [x] CompleteQuestObjective should default to completing the objective we're inside of.
-- [x] Added TalkAndCompleteQuestObjective to simplify simple setup.
--
-- I don't think we should use AddConvo directly. Maybe make an Entrypoint() thing for it instead?
--
-- Invisible, PromptString need implementation to enable.
--
-- Should rename node -> actor? Something to indicate that they're who you're
-- talking to/questing with. Already renamed node -> root when it's the root
-- node (QuestCentral).
...
function Q:Quest_Complete()
	...
	-- activate 'meet dojo master' quest here
	self:GetQuestManager():SpawnQuest("twn_unlock_cannon")
...
local function StartNewGame(quest)
...
local function _GoToDungeon()
	local RoomLoader = require "roomloader"
	local altMapGenID = 1 -- treemon_forest_tutorial1
	local ascension = 0
...
Q:AddObjective("quest_intro")
	...
	:OnActivate(function(quest)
		 -- if you're in a networked game (or cheating), don't do this cinematic.
		if TheSaveSystem.cheats:GetValue("skip_new_game_flow") or not TheNet:IsGameTypeLocal() then
			quest:Complete("quest_intro")
