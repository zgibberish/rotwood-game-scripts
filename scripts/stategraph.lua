...
StateGraphRegistry = Class(function(self)
	...
	self.statetags_nrbits = #self.statetags
	TheLog.ch.StateGraphRegistry:printf("%d bits required to represent state tags", self.statetags_nrbits)
	assert(self.statetags_nrbits <= 64, "Too many state tags to trivially serialize")
end)
...
function StateGraphRegistry:SerializeStateTags(tags)
	local tag_bits = 0
	for tag,_ in pairs(tags) do
		local bit = self.statetags_reverse[tag] and self.statetags_reverse[tag] - 1
		if bit then
			tag_bits = tag_bits | (1 << bit)
		else
			TheLog.ch.StateGraphRegistry:printf("Warning: Unable to find id for tag %s (it will not be serialized)", tag)
		end
	end
	return tag_bits
end
...
function StateGraphRegistry:DeserializeStateTags(tag_bits, out_tags)
	...
	while tag_bits > 0 do
		local bitmask = 1 << bit
		if bitmask & tag_bits == bitmask then
			local statename = self.statetags[bit + 1]
			out_tags[statename] = true
		end
		tag_bits = tag_bits & ~bitmask
		bit = bit + 1
	end
...
function StateGraphInstance:_OnNetSerializeTransferable()
	...
	if self:HasStateTag("attack_hold") and not self.inst:HasTag("no_state_transition") and self.inst.components.attacktracker:GetActiveAttack() then
		...
		e:SerializeUInt(math.max(0, self:GetTimeoutTicks() - self:GetTicksInState()), TimeoutTicksNrBits)
...
function StateGraphInstance:OnNetSerialize()
	...
	if has_registry_data then
		...
		e:SerializeUInt(stateid, stateid_nrbits)
		e:SerializeUInt(SGRegistry:SerializeStateTags(self.tags), SGRegistry:GetStateTagNrBits())
...
function StateGraphInstance:OnNetDeserialize()
	...
	if has_registry_data then
		...
		local statetag_bits = e:DeserializeUInt(SGRegistry:GetStateTagNrBits())
		if SGRegistry:HasHint(self.sg.name, StateGraphRegistry.Hints.SerializeTicksInState) then
		...
		SGRegistry:DeserializeStateTags(statetag_bits, self.tags)
