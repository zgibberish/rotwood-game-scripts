...
function SaveSystem:LoadCharacterAsPlayerID(slot, id)
	...
	local local_players = TheNet:GetLocalPlayerList()
	-- If there is only one local player (or the local player list doesn't exist yet), that player must be the "main" local player.
	if not local_players or #local_players == 1 then
		Profile:SetLastSelectedCharacterSlot(slot)
	end
