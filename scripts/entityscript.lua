...
function EntityScript:Pause()
	...
	if not self.inlimbo and self.AnimState ~= nil then
		self.AnimState:Pause()
	end
	if self.sg ~= nil then
...
function EntityScript:__tostring()
	...
	return string.format("%d - %s%s%s", self.GUID, self.name or self.prefab or "", label, self.inlimbo and "(LIMBO)" or "")
end
...
function EntityScript:NetSerialize()
	...
	--	print("serializing " .. self.entity:GetPrefabName())
		...
		self.entity:SerializeBoolean(self.inlimbo)
...
function EntityScript:NetDeserialize()
	...
	if self.entity:HasNewDeserializationData() then
		...
		local in_limbo = self.entity:DeserializeBoolean()
		if in_limbo ~= self.inlimbo then
			if in_limbo then
				self:RemoveFromScene()
			else
				self:ReturnToScene()
