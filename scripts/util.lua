...
function SetFormattedText( textnode, str )
	...
	str = ApplyFormatting( textnode, str )
	if textnode then
...
-- <p img='images/ui_ftf_shop/displayvalue_up.tex' color=40AB38 scale=1.2 advancement=75.0>
...
function ApplyFormatting( textnode, str )
	textnode:ClearMarkup()
	local j, k, sel, attr
	local spans = {}
	...
	repeat
		...
		if j then
			...
			elseif sel == "p" then
				...
				-- example options:
				-- <p img=folder/image.tex scale=1.0 advancement=75.0>
				local img, scale, color = ParseImageInformation( attr )
				local bind = string.match(attr, [[bind=['’]([^'’]+)]]) -- case sensitive
				if bind then
					img = TheInput:GetTexForControlName(bind)
			...
			elseif sel == "c" then
				...
				local img_path = TheInput:GetDeviceImageAtlas(device_id)
				if img then
					if img_path then
						validmarkup = true
