...
MetaProgress.Reward = Class(function(self, category, slot, id, ...)
	local def = category.Items[slot][id]
	self.slot = slot
	self.def = def
	-- assert(self.def._default_unlock, string.format("Tried to add a def to MetaProgress that is unlocked by default! [%s]", id))
end)
...
function MetaProgress.GetRewardForLevel(def, level)
	if level == 0 or #def.rewards < level then
		return nil
	end
