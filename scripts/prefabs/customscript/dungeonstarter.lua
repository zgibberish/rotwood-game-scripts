...
local function OnCancelRun(inst, player)
	if player and player:IsLocal() then
		local requestingPlayerID, mode, arenaWorldPrefab, regionID, locationID, seed, altMapGenID, ascensionLevel, seqNr = TheNet:GetRequestedRunData()
...
local function CanInteractDefault(inst, player, is_focused)
	local playerID, mode, arenaWorldPrefab, regionID, locationID, seed, altMapGenID, ascensionLevel, seqNr = TheNet:GetRequestedRunData()
...
local function SetInteractableToCancel(inst)
	...
	inst.components.interactable:ForceClearInteraction()
end
...
function dungeonstarter.ConfigureDungeonStarter(inst, opts)
	...
	if TheWorld:HasTag("town") then
		...
		inst.components.startrunportal.radius = 4.5
