...
local function MakeAutogenWorld(name, params)
	...
	local function fn(prefabname)
		...
		if params.layout ~= nil then
			--Load map layout
			inst.layout = require("map/layouts/"..params.layout)

			local maplayout = MapLayout(inst.layout)
			local layouttiles = maplayout:GetGroundLayer()

			local from_tile_group = inst.layout.tilesets[1].name
			...
			if not tile_id_resolver:IsRemapper() then
				TheLog:print("Warning: Cannot remap from tile group '"..from_tile_group.."' to '"..to_tile_group.."'.")
				to_tile_group = from_tile_group
			end

			inst.tilegroup = GroundTiles.TileGroups[to_tile_group]
			assert(TheSceneGen or maplayout.tilegroup == inst.tilegroup)
			...
			assert(layouttiles.width == inst.layout.width and layouttiles.height == inst.layout.height)
			...
			local bounds = maplayout:GetWorldspaceBounds()
			if bounds then
