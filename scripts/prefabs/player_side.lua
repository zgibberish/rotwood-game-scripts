...
local function OnSetOwner(inst)
	...
	local on_player_set = function()
		...
		if inst.components.progresstracker:GetValue("total_num_runs") == 0 then
			TheWorld:PushEvent("new_game_started")
		end
	...
	if TheSaveSystem.cheats:GetValue("skip_new_game_flow") then
		...
		if not Profile:GetLastSelectedCharacterSlot() then
			...
			Profile:SetLastSelectedCharacterSlot(0)
		end
	end
	...
	if TheSaveSystem.active_players:GetValue("quick_start") then
		...
		if inst:IsOnlyLocalPlayer() then
			TheSaveSystem:LoadCharacterAsPlayerID(Profile:GetLastSelectedCharacterSlot(), playerID)
...
local function GetCustomUserName(inst)
	...
	-- For MP we don't want everyone to be named 'Hunter Hunter'
	-- if RELEASE_CHANNEL == "demo" then
	-- 	return "Hunter"
	-- end

	local playerID = TheNet:IsInGame() and inst.Network:GetPlayerID()
...
local function RefreshEmotes(inst)
	...
	if species == "ogre" then
		inst.components.playeremoter:EquipEmote(3, "emote_ogre_charged_jump")
	elseif species == "canine" then
		inst.components.playeremoter:EquipEmote(3, "emote_mammimal_howl")
	elseif species == "mer" then
		inst.components.playeremoter:EquipEmote(3, "emote_amphibee_bubble_kiss")
	end
	...
	if weapon then
		if weapon == WEAPON_TYPES.HAMMER then
			inst.components.playeremoter:EquipEmote(7, "emote_hammer_twirl")
		elseif weapon == WEAPON_TYPES.POLEARM then
			inst.components.playeremoter:EquipEmote(7, "emote_polearm_twirl")
		elseif weapon == WEAPON_TYPES.CANNON then
			inst.components.playeremoter:EquipEmote(7, "emote_cannon_twirl")
...
local function MakePlayerCharacter(name, customprefabs, customassets, common_postinit, master_postinit)
	...
	local function fn(prefabname)
		...
		inst.AnimState:SetSilhouetteColor(0,0,0,PLAYER_SILHOUETTE_ALPHA)
		...
		inst.components.playeremoter:EquipEmote(2, "emote_pump")
