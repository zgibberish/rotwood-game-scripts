...
local function fn(name, opts)
	...
	inst._onplayerentered = function()
		if inst.components.rotatingdrop:GetDropCount() ~= TheNet:GetNrPlayersOnRoomChange() then
			local drops = _BuildDropsTable(inst)
			inst.components.rotatingdrop:SpawnDrops(drops)
			inst.components.rotatingdrop:PrepareToShowDrops()
		end
	end
	inst:ListenForEvent("playerentered", inst._onplayerentered, TheWorld)
	inst:ListenForEvent("despawn", function(inst)
		inst:RemoveEventCallback("playerentered", inst._onplayerentered, TheWorld)
	end)
