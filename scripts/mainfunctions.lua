...
local DebugConsole = require "dbui.debug_console"
require "knownerrors"
require "scheduler"
--require "skinsutils"
...
function OnEntityBecameLocal(guid)
	local inst = Ents[guid]
	if inst ~= nil then
...
local function CheckControllers()
	...
	local sawPopup = true -- Profile:SawControllerPopup()
...
function OnNetworkDisconnect(message, should_reset, force_immediate_reset, details)
	...
	if should_reset and InGamePlay() then
		message = message .. "\n" .. STRINGS.UI.NETWORKDISCONNECT.RESET
	end
		...
		if TheFrontEnd.fadedir == false then
			...
			TheFrontEnd.fadecb = function()
				...
				TheFrontEnd:PushScreen(ConfirmDialog(nil, nil, true,
					...
					):SetYesButton(STRINGS.UI.NETWORKDISCONNECT.OK, function()
						doquit(should_reset)
					end)
					:HideNoButton()
					:HideArrow()
