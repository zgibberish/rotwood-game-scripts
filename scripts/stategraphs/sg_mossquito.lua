...
local function DropDeathAoE(inst)
	--[[if TheWorld.components.roomclear:IsRoomComplete() then
		return
	end]]
	local projectile = SpawnPrefab("mossquito_heal_drop", inst)
	projectile:Setup(inst)

	projectile.owner = nil -- Fix(?) to get healing numbers to appear. The owner is already dead when healing, so it will fail an IsLocal() check in PlayerHud:MakeDamageNumber()
	projectile.components.hitbox:SetHitFlags(HitGroup.CREATURES) -- This must be called after Setup() in order for this projectile to hit other enemies.

	local offset_x = 5/150

	if inst.Transform:GetFacing() == FACING_LEFT then
		offset_x = offset_x * -1
	end

	local offset = Vector3(offset_x, 300/150, 0)
	local x, z = inst.Transform:GetWorldXZ()
	projectile.Transform:SetPosition(x + offset.x, offset.y, z + offset.z)
	--projectile:PushEvent("drop", inst:GetPosition())
	projectile.sg:GoToState("land", projectile:GetPosition())

	--[[local burst = SGCommon.Fns.SpawnAtDist(inst, "mosquito_trail_burst", 0)
	burst:DoTaskInAnimFrames(15, function()
		burst.components.particlesystem:StopThenRemoveEntity()
	end)
	burst:ListenForEvent("onremove", function() burst:Remove() end, projectile)]]
...
local function OnDeath(inst, data)
	--Spawn death fx
	local fx_name = inst:HasTag("elite") and "death_mossquito_elite" or "death_mossquito"
	EffectEvents.MakeEventFXDeath(inst, data.attack, fx_name, { y = 1.5 })

	inst.components.lootdropper:DropLoot()
	if inst:HasTag("elite") then
		DropDeathAoE(inst) -- TODO(jambell): launch a complex projectile that spawns this
	end
end
...
local states =
{
	State({
		...
		onupdate = function(inst)
			if inst.sg.statemem.hitting then
				inst.components.hitbox:PushBeam(0.00, 2.25, 0.50, HitPriority.MOB_DEFAULT)
			end
		end,
	...
	State({
		...
		{
			-- Code Generated by PivotTrack.jsfl
			FrameEvent(2, function(inst) inst.Physics:MoveRelFacing(26/150) end),
			FrameEvent(5, function(inst) inst.Physics:MoveRelFacing(8/150) end),
			-- End Generated Code
		},
