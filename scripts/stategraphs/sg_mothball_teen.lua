...
local states =
{
		...
		timeline =
		{
			FrameEvent(0, function(inst)
				...
				if inst:HasTag("elite") then
					local projectile = SGCommon.Fns.SpawnAtAngleDist(inst, "mothball_teen_projectile_elite", 1, -45)
					if projectile == nil then return end
					SetupProjectile(projectile)

					local projectile2 = SGCommon.Fns.SpawnAtAngleDist(inst, "mothball_teen_projectile2_elite", 1, 45)
					if projectile2 == nil then return end
					SetupProjectile(projectile2)

				else
					local projectile = SGCommon.Fns.SpawnAtDist(inst, "mothball_teen_projectile", 1)
					if projectile == nil then return end
					SetupProjectile(projectile)
				end
			end),
	...
	State({
		...
		onupdate = function(inst)
			if inst.sg.statemem.escape_attacking then
		...
		timeline =
		{
			FrameEvent(2, function(inst) inst.sg.statemem.escape_attacking = true end)
		},
		events =
		{
