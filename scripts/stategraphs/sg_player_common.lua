...
local DoControlFn =
{
	...
	["interact"] = function(inst, data)
		if data.target ~= nil
			and data.target.components.interactable ~= nil
			and data.target.components.interactable:CanPlayerInteract(inst, true)
		then
			...
			if nextstate ~= nil then
				inst.components.playercontroller:FlushControlQueueAt(data)
				local target_pos = data.target.components.interactable:GetInteractionWorldPosition(inst)
				if target_pos then
...
function SGPlayerCommon.Fns.SetCanMove(inst)
	...
	inst.sg.statemem.canmove = true
end

function SGPlayerCommon.Fns.UnsetCanMove(inst)
	dbassert(inst.sg:HasStateTag("busy")) --just to make sure we're using this correctly
	inst.sg.statemem.canmove = nil
end
...
function SGPlayerCommon.Fns.SetRollPhysicsSize(inst)
	...
	--inst.HitBox:SetNonPhysicsRect(inst.sg.mem.preroll_physicssize)
	local new_size = math.max(0.1, inst.sg.mem.preroll_physicssize * 0.2) -- if we go smaller than this, the game hard-crashes
	inst.Physics:SetSize(new_size)
end
...
function SGPlayerCommon.States.AddRollStates(states)
	...
	states[#states + 1] = State({
		...
		{
			--physics
			FrameEvent(0, function(inst) SGCommon.Fns.SetMotorVelScaled(inst, inst.sg.statemem.speed * 0.65) end),
			FrameEvent(2, function(inst) SGCommon.Fns.SetMotorVelScaled(inst, inst.sg.statemem.speed * 0.55) end),
			FrameEvent(3, function(inst) SGCommon.Fns.SetMotorVelScaled(inst, inst.sg.statemem.speed * 0.45) end),
			FrameEvent(4, function(inst) SGCommon.Fns.SetMotorVelScaled(inst, inst.sg.statemem.speed * 0.35) end),
			FrameEvent(5, function(inst) SGCommon.Fns.SetMotorVelScaled(inst, inst.sg.statemem.speed * 0.25) end),
	...
	states[#states + 1] = State({
		...
		timeline =
		{
			...
			FrameEvent(2, function(inst) SGCommon.Fns.SetMotorVelScaled(inst, inst.sg.statemem.velocity * 0.175) end),
			FrameEvent(4, function(inst) SGCommon.Fns.SetMotorVelScaled(inst, inst.sg.statemem.velocity * 0.1) end),
			FrameEvent(6, function(inst) SGCommon.Fns.SetMotorVelScaled(inst, inst.sg.statemem.velocity * 0.05) end),
...
function SGPlayerCommon.States.AddDepositCurrencyState(states)
	states[#states + 1] = State({
		...
		onupdate = function(inst)
			...
			local interactable = GetInteractable(inst.sg.mem.interact_target)
			if interactable then
				local can, reason = interactable:CanPlayerInteract(inst)
				if can then
					interactable:StartInteract(inst)
					for _ = 1, inst.sg.statemem.deposits_per_proc do
						interactable:PerformInteract(inst)
						-- Bypass the interactable to poll the client directly. We are in control of our interactable
						-- status, but we need to terminate when the client says so.
						can, reason = interactable.caninteractfn(interactable, inst)
						if not can then
							break
						end
					end
					-- Don't make the interaction last multiple frames -- clear it up immediately this frame.
					interactable:ClearInteract(inst, true)
				end
				if not can then
			...
			-- The interaction has happened, so reset.
		...
		onexit = function(inst)
			...
			if target_sg then
				if target_sg.mem.is_interacting then
