			...
			EventHandler("oncontrol_music", function(inst, data)
				...
				elseif inst.sg.mem.failnextpress then
					...
					if not succeeded then
						inst.sg.mem.failnextpress = true
						print("Wrong button!")
						print("Pressed:", data.control.displayname)
						print("Requested:", inst.sg.mem.correctbutton.displayname)
					end
				end
				...
				if succeeded then
					...
					TheDungeon.HUD:MakeCookingButton({ target = inst, button = "<p img='images/ui_dst/checkmark.tex'>"})--"YES!", y_offset = 2000 })
				else
					print("Fail!")
					inst.sg.mem.presssucceeded = false
					inst.AnimState:PlayAnimation("knockback")
					inst.AnimState:PushAnimation("idle", true)
					TheDungeon.HUD:MakeCookingButton({ target = inst, button = "<p img='images/square.tex'>"})--"NO!!!", y_offset = 2000 })
