...
local OLD_PROJECTILE_UPDATE_OVER_COUNT <const> = 8
local OLD_PROJECTILE_UPDATE_PERIOD = 0.25

local function OldProjectileCleanupPass(inst)
	if not inst.sg.mem or not inst.sg.mem.old_projectile_ids then
		return
	end

	local actions = 0
	for entID,_ in pairs(inst.sg.mem.old_projectile_ids) do
		local entGUID = TheNet:FindGUIDForEntityID(entID)
		if not entGUID then
			-- over-count to confirm no one took this ent at the last moment
			inst.sg.mem.old_projectile_ids[entID] = inst.sg.mem.old_projectile_ids[entID] + 1
			if inst.sg.mem.old_projectile_ids[entID] >= OLD_PROJECTILE_UPDATE_OVER_COUNT then
				TheLog.ch.Shotput:printf("Old Projectile Cleanup Task: Confirmed EntityID %d removed", entID)
				inst.sg.mem.old_projectile_ids[entID] = nil
				actions = actions + 1
			-- else
			-- 	TheLog.ch.Shotput:printf("Old Projectile Cleanup Task: Waiting for EntityID %d to timeout (%d)",
			-- 		entID, inst.sg.mem.old_projectile_ids[entID])
			end
		else
			local ent = Ents[entGUID]
			if ent:IsLocal() and not ent:IsInDelayedRemove() then
				TheLog.ch.Shotput:printf("Old Projectile Cleanup Task: Removing local %s EntityID %d", ent, entID)
				ent:Remove()
				if inst.sg.mem.old_projectile_ids[entID] > 0 then
					TheLog.ch.Shotput:printf("Old Projectile Cleanup Task: Resetting timeout for %s EntityID %d", ent, entID)
				end
				inst.sg.mem.old_projectile_ids[entID] = 0
				actions = actions + 1
			elseif not ent:IsLocal() then
				TheLog.ch.Shotput:printf("Old Projectile Cleanup Task: Removing remote %s EntityID %d", ent, entID)
				ent:TakeControl()
				ent:Remove()
				if inst.sg.mem.old_projectile_ids[entID] > 0 then
					TheLog.ch.Shotput:printf("Old Projectile Cleanup Task: Resetting timeout for %s EntityID %d", ent, entID)
	...
	-- if actions > 0 then
	-- 	TheLog.ch.Shotput:printf("Old Projectile Cleanup Task actions: %d", actions)
	-- end
end

local function StartOldProjectileCleanupTask(inst)
	if not inst.sg.mem.old_projectile_cleanup_task then
		TheLog.ch.Shotput:printf("Old Projectile Cleanup Task STARTING...")
		inst.sg.mem.old_projectile_cleanup_task = inst:DoPeriodicTask(OLD_PROJECTILE_UPDATE_PERIOD, function()
			-- TheLog.ch.Shotput:printf("Old Projectile Cleanup Periodic Task WORKING...")

			if not inst.sg.mem.old_projectile_ids or not next(inst.sg.mem.old_projectile_ids) then
				TheLog.ch.Shotput:printf("Old Projectile Cleanup Task DONE.")
				inst.sg.mem.old_projectile_cleanup_task:Cancel()
				inst.sg.mem.old_projectile_cleanup_task = nil
				return
			end

			OldProjectileCleanupPass(inst)
		end)
	end
end

-- TODO: networking2022, journal entity removal so we don't need bespoke stuff like this
local function TestForHandledProjectileRemoval(inst, projectile)
	inst.sg.mem.old_projectile_ids = inst.sg.mem.old_projectile_ids or {}

	StartOldProjectileCleanupTask(inst)

	if inst.sg.mem.old_projectile_ids[projectile.Network:GetEntityID()] then
		TheLog.ch.Shotput:printf("Duplicate handled projectile.  Ignoring...")
		return true
	else
		-- Store entity id instead of ref since it could get removed and then recreated with a diff ref but same entity id
		inst.sg.mem.old_projectile_ids[projectile.Network:GetEntityID()] = 0 -- number of updates confirmed removed
		return false
	end
end
...
local function OnCatch(inst, projectile)
	...
	local already_handled = TestForHandledProjectileRemoval(inst, projectile)
	projectile:Remove()
	...
	State({
		...
		onenter = function(inst)
			...
			local _on_loadout_change_imminent = function()
				...
				OldProjectileCleanupPass(inst)
				if inst.sg.mem.old_projectile_cleanup_task then
					inst.sg.mem.old_projectile_cleanup_task:Cancel()
					inst.sg.mem.old_projectile_cleanup_task = nil
				end
