...
local states =
{
	State({
		...
		onenter = function(inst)
			-- Use default tuning to wait for player to get close and wake us.
	...
	State({
		...
		timeline =
		{
			...
			FrameEvent(7, function(inst)
				if inst.sg.statemem.right then
					inst.components.hitbox:PushOffsetBeam(1.5, 4, 1.5, -2, HitPriority.MOB_DEFAULT)
					inst.components.hitbox:PushOffsetBeam(3, 5, 1.5, -1, HitPriority.MOB_DEFAULT)
				else
					inst.components.hitbox:PushOffsetBeam(-6, -3.5, 1.5, -2, HitPriority.MOB_DEFAULT)
					inst.components.hitbox:PushOffsetBeam(-7, -5, 1.5, -1, HitPriority.MOB_DEFAULT) -- Because of asymmetry, these numbers aren't the same as right facing
				end
			end),
			...
			FrameEvent(8, function(inst)
				if inst.sg.statemem.right then
					...
					inst.components.hitbox:PushOffsetBeam(-5, -2, 1.5, -2, HitPriority.MOB_DEFAULT)
				end
			end),
