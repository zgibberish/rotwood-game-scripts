...
local states =
{
	...
	State{
		...
		timeline =
		{
			FrameEvent(4, function(inst)
				inst.components.hitbox:StartRepeatTargetDelay()
				inst.components.hitbox:PushBeam(-0.3, 3, 1.5, HitPriority.MOB_DEFAULT)
			end),
			FrameEvent(5, function(inst)
				inst.components.hitbox:PushBeam(-0.3, 1, 1, HitPriority.MOB_DEFAULT)
			end),
			FrameEvent(8, function(inst)
				inst.Physics:Stop()
			end)
	...
	State{
		...
		timeline =
		{
			FrameEvent(4, function(inst)
				inst.components.hitbox:StartRepeatTargetDelay()
				inst.components.hitbox:PushBeam(-2 , 0, 1, HitPriority.MOB_DEFAULT)
			end),
			FrameEvent(5, function(inst)
				inst.components.hitbox:PushOffsetBeam(0 , 2.5, 1, -1, HitPriority.MOB_DEFAULT)
			end),
			FrameEvent(6, function(inst)
				inst.components.hitbox:PushOffsetBeam(0 , 2.5, 1, -1, HitPriority.MOB_DEFAULT)
			end),
