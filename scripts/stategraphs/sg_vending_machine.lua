...
local active_states = Lume(ACTIVE_PROTO_STATES):enumerate(function(funding_name, proto_state)
	return State {
		...
		onupdate = function(inst)
			-- sound stuff
			local deposited_total = inst.components.vendingmachine:GetTotalDeposited()
			local percent_funded = deposited_total / inst.components.vendingmachine.cost
			if inst.sg.mem.interacting_sound_handle then
				soundutil.SetInstanceParameter(inst, inst.sg.mem.interacting_sound_handle, "progress", percent_funded)
			end
