...
--- Create a bound from four values.
-- KLEI: we often pass bounding boxes from native as four values.
-- @treturn bound2 out
function bound2.from_values(min_x, min_y, max_x, max_y)
	return new(vec2(min_x, min_y), vec2(max_x, max_y))
end
...
--- Check the validity of the bound.
-- @tparam bound2 a bound
-- @treturn bool true if the bound constitutes a non-zero area, false otherwise
function bound2.is_valid(a)
	return a.min.x < a.max.x and a.min.y < a.max.y
end
...
local Bound2 = setmetatable({}, bound2_mt)
return Bound2
