...
local ServerPreferences = require "serverpreferences"
...
local function PopulateWorld(savedata, profile)
	...
	if world.components.propmanager ~= nil then
		...
		if world.layout ~= nil then
			local maplayout = MapLayout(world.layout)
			-- Skip the ground layer
			for i = 2, #world.layout.layers do
				local objects = world.layout.layers[i].objects
				if objects ~= nil then
					for j = 1, #objects do
						local object = objects[j]
						local record = maplayout:ConvertLayoutObjectToSaveRecord(object)
		...
		world.components.propmanager:SpawnStaticProps(world.layout)
...
TheNetUtils = {}
TheNetUtils.ServerPreferences = ServerPreferences()
TheNetUtils.ProfanityFilter = ProfanityFilter()

TheNetUtils.ServerPreferences:Load( function() end )
