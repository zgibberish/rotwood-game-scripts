...
local ServerPreferences = Class(function(self)
    self.persistdata = {} 
	...
	if self.profanityservers[server_id] ~= nil then
		...
		if hide then
			...
			if self.persistdata[server_id] ~= nil and self.persistdata[server_id].hidename then
				...
				if table.numkeys(self.persistdata[server_id].hidename) <= 1 then -- 1 for lastseen
	...
	if Profile:GetProfanityFilterServerNamesEnabled() then
		...
		if servers ~= nil then
			for i, server in ipairs(servers) do
				local server_id = MakeServerID(server)
				if self.profanityservers[server_id] == nil and (TheNetUtils.ProfanityFilter:HasProfanity(server.name) or TheNetUtils.ProfanityFilter:HasProfanity(server.description)) then
			...
			if self.profanityservers[server_id] == nil and (TheNetUtils.ProfanityFilter:HasProfanity(name) or TheNetUtils.ProfanityFilter:HasProfanity(TheNet:GetServerDescription())) then
