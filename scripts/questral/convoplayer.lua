...
function ConvoPlayer:FlagAsUnimportantConvo()
    ...
    self:Talk("TALK.TALK_UNIMPORTANT")
    return self
end
...
function ConvoPlayer:PushAgentHub()
    self:Loop(function()
        ...
        if #self.current_options == 0 then
            ...
            self:Opt("TALK.OPT_LEAVE"):Back():EndLoop():Fn(function() self.screen:StopTalking() end)
        end
    end)
end
...
function ConvoPlayer:GrantMoney(amt)
    self:Talk("TALK.TALK_GAIN_MONEY", amt)
...
function ConvoPlayer:GrantStatProgress( amt, category, params )
    ...
    if cmp then
        ...
        self:Talk("TALK.TALK_GAIN_STAT_PROGRESS", amt, CmpCharacterStats:LOC(category))
    end
    return self
end
...
function ConvoPlayer:DeltaTether(amt)
    ...
    if change > 0 then
        self:Talk("TALK.TALK_GAIN_TETHER", change)
    elseif change < 0 then
        self:Talk("TALK.TALK_LOSE_TETHER", -change)
    end
...
function ConvoPlayer:AddBack()
    return self:Opt("TALK.OPT_BACK"):Back()
end

function ConvoPlayer:AddEnd(dlg)
    return self:Opt(dlg or "TALK.OPT_LEAVE"):Back():End()
end

function ConvoPlayer:AddEndLoop()
    return self:Opt("TALK.OPT_BACK"):EndLoop():Back()
end
