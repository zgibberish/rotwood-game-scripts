...
function Quest.CreateRecurringChat()
    local Q = Quest.Create(Quest.QUEST_TYPE.s.CHAT, contentutil.BuildClassNameFromCurrentFile())
    Q:AddCast("giver")
...
function Quest.CreateMainQuest()
    local Q = Quest.Create(Quest.QUEST_TYPE.s.MAIN, contentutil.BuildClassNameFromCurrentFile())
    Q:AddCast("giver")
    return Q
end
...
function Quest.CreateJob()
    local Q = Quest.Create(Quest.QUEST_TYPE.s.JOB, contentutil.BuildClassNameFromCurrentFile())
    Q:AddCast("giver")
    return Q
end
...
function Quest:TitleString(str)
    assert(self:is_class(), "Don't call this on an instance")
    self:AddStrings{TITLE = kstring.trim(str)}
    return self
end
...
function Quest:DescString(str)
    assert(self:is_class(), "Don't call this on an instance")
    self:AddStrings{DESC = kstring.trim(str)}
    return self
end
...
function Quest:OnActivate( root )
    ...
    for event in pairs(all_events) do
        ...
        -- self.inst:ListenForEvent(event, fn, self:GetQC():GetEventEntity())
    end
...
-- TODO(dbriscoe): See QuestManager:GetPlayer.
...
function Quest:_ListenForEventsOnCast(cast_member)
    ...
    TheWorld:PushEvent("cast_member_filled", cast_member)
...
function Quest:Debug_Cancel()
    ...
    assert(self.forbid_serialize, "Why Debug_Cancel() a quest that wasn't debug spawned?") -- see MarkAsDebug
    return self:Cancel()
end
