...
local USE_SETTINGS_FILE = not Platform.IsPS4()
...
function PlayerProfile:Reset()
	...
	self.persistdata.suppress_decor_props = false
...
function PlayerProfile:GetSkins()
	...
	for prefab, skins in pairs(PREFAB_SKINS) do 
		...
		owned_skins = table.appendarrays({}, owned_skins, skins)
	end
...
-- Store the last slot the player used for quick start purposes
function PlayerProfile:GetLastSelectedCharacterSlot()
   	return self.persistdata.last_selected_character_slot
end

function PlayerProfile:SetLastSelectedCharacterSlot(slot)
    self.persistdata.last_selected_character_slot = slot
    self.dirty = true
    self:Save()
end
...
function PlayerProfile:GetValue(name, default)
	local val = self.persistdata[name]
	if val == nil then
		val = default
	end
	return val
...
function PlayerProfile:SetBloomEnabled(enabled)
 	if USE_SETTINGS_FILE then
		TheGameSettings:Set("graphics.bloom", enabled)
	else
...
function PlayerProfile:SetRimLightingEnabled(enabled)
	if USE_SETTINGS_FILE then
		TheGameSettings:Set("graphics.rimlighting", enabled)
	else
		self:SetValue("rimlighting", enabled)
		self.dirty = true
	end
end
...
function PlayerProfile:GetBloomEnabled()
 	if USE_SETTINGS_FILE then
		return TheGameSettings:Get("graphics.bloom")
	else
		return self:GetValue("bloom")
	end
end

function PlayerProfile:GetRimLightingEnabled()
	if USE_SETTINGS_FILE then
		return TheGameSettings:Get("graphics.rimlighting")
	else
		return self:GetValue("rimlighting")
	end
end
...
function PlayerProfile:Save(callback)
    TheLog.ch.SaveLoad:print("SAVING PlayerProfile")
    if self.dirty then
        local str = json.encode(self.persistdata)
...
function PlayerProfile:Set(str, callback, minimal_load)
    if str == nil or string.len(str) <= 0 then
		...
		if self.persistdata.favorite_mods == nil then
 	    ...
 	    if USE_SETTINGS_FILE then
			...
			if self.persistdata.volume_ambient ~= nil and self.persistdata.volume_sfx ~= nil and self.persistdata.volume_music ~= nil then
				TheLog.ch.Audio:print("Copying audio settings from profile to settings.ini")
		...
		TheLog.ch.Audio:print("volumes", amb, sfx, music )
		...
		local bloom_enabled = TheGameSettings:Apply("graphics.bloom")
		local rimlighting_enabled = TheGameSettings:Apply("graphics.rimlighting")
		TheLog.ch.Settings:printf("graphics.bloom=%s, graphics.rimlighting=%s", tostring(bloom_enabled), tostring(rimlighting_enabled))

		local vibration_enabled = TheGameSettings:Apply("gameplay.vibration")
		TheLog.ch.Settings:printf("gameplay.vibration=%s", tostring(vibration_enabled))
		if USE_SETTINGS_FILE then
			-- Copy over old settings
			if self.persistdata.bloom ~= nil and self.persistdata.HUDSize ~= nil then
...
function PlayerProfile:GetFeedbackName()
	if self:GetValue("feedback_name") ~= nil then
		return self:GetValue("feedback_name")
	end
end

function PlayerProfile:SetFeedbackName(name)
	self:SetValue("feedback_name", name)
	self.dirty = true
	self:Save()
end
