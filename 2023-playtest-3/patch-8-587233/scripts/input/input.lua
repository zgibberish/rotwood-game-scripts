...
-- TODO(input): Do we sometimes use 0 for this kind of id?
local KBM_DEVICE_ID <const> = "kbd"
...
local Input = Class(function(self)
	...
	self.active_inputs = {} -- map device to active state about each control.
	self.active_inputs[KBM_DEVICE_ID] = {}
	...
	self.enable_logging = false -- activates self:LogPrintf spew
...
function Input:LogPrintf(...)
	if self.enable_logging then
		TheLog.ch.Input:printf(...)
	end
end
	...
	if device_type == "gamepad" then
		if device_id == -1 then
			...
			for k,v in pairs(self.gamepads) do
				if not self.gamepadowners[k] then
					self.gamepadowners[k] = ent
					self:LogPrintf("RegisterDeviceOwner: Gamepad id=%d name=%s assigned to entity GUID %d", k, v.name, ent.GUID)
					return k
				elseif self.gamepadowners[k] == ent then
					self:LogPrintf("RegisterDeviceOwner: Gamepad id=%d name=%s already assigned to entity GUID %d", k, v.name, ent.GUID)
					return k
				end
			end
		end
		...
		if self.gamepads[device_id] then
			if not self.gamepadowners[device_id] then
				...
				self:LogPrintf("RegisterDeviceOwner: Gamepad id=%d name=%s assigned to entity GUID %d", device_id, self.gamepads[device_id].name, ent.GUID)
				return device_id
			elseif self.gamepadowners[device_id] == ent then
				self:LogPrintf("RegisterDeviceOwner: Gamepad id=%d name=%s already assigned to entity GUID %d", device_id, self.gamepads[device_id].name, ent.GUID)
				return device_id
			end
		end
...
function Input:UnregisterDeviceOwner(device_type, device_id)
	if device_type == "gamepad" then
		...
		if ent then
			self:LogPrintf("UnregisterDeviceOwner %s for device_id=%d", tostring(ent), device_id)
...
function Input:SetGamepadBinds(src_bindings)
	...
	self.gamepad_lookup = {} -- k:control (displayname, activebindings) (table) v:binding entry (button, control, etc.) (table)
...
function Input:CacheController()
	self.controllerid_cached = Platform.IsNotConsole() and (TheInputProxy:GetLastActiveControllerIndex() or 0) or nil
...
function Input:ControllerAttached()
	...
	return Platform.IsConsole() or TheInputProxy:IsAnyControllerActive()
end

function Input:ControllerConnected()
	--V2C: didn't cache this one because it's not used regularly
	return Platform.IsConsole() or TheInputProxy:IsAnyControllerConnected()
end
...
function Input:AddGeneralControlHandlerForGamepad(device_id, fn)
	if self.per_gamepad_oncontrol[device_id] then
		self:LogPrintf("Using general gamepad control handler for device=" .. device_id)
...
function Input:RemoveGeneralControlHandlerForGamepad(device_id, handler)
	if self.per_gamepad_oncontrol[device_id] and handler then
		self:LogPrintf("Removing general gamepad control handler for device=" .. device_id)
...
function Input:AddGeneralControlHandler(fn)
	self:LogPrintf("Using general control handler")
...
function Input:DumpAnalogControlValue(control, device_type, device_id)
	if control then
		...
		if state then
			...
			local analog = state.val or 0
			TheLog.ch.Input:printf("DumpAnalogControlValue control=%s digital=%1.3f analog=%1.3f", control.displayname, digital, analog)
		end
	end
end
...
function Input:OnUpdate(dt)
	...
	for k,v in pairs(self.control_state) do
		if v.down and k.repeat_rate then
			...
			if v.rep_time <= 0 then
				...
				self:DoControlRepeat(k, "keyboard")
			end
		end
	end
	...
	if newCursorVisible ~= self.cursorvisible then
		...
		self:LogPrintf("Cursor Visibility Changed: %s", tostring(newCursorVisible))
	end
end
...
-- Prefer GetLabelForControl which returns icon tags using the active device.
...
function Input:GetLabelForControl(control)
	local tex = self:GetTexForControlName(control.key)
	if tex then
...
function Input:GetTexForControlName(control_key, device_type)
	...
	if not button_id then
		self:LogPrintf("Failed to find button_id for control '%s' atlas '%s' device '%s'", control_key, atlas, device_type)
		return
	end
	return self:_GetTexForButtonId(button_id, atlas)
end

function Input:_GetTexForButtonId(button_id, atlas)
	if not atlas then
		local device_id = 1 -- TODO(input): Pass player-specific device instead.
		atlas = self:GetDeviceImageAtlas(device_id)
	end
...
-- Slightly different from self.gamepads[device_id].name because it returns
-- simpler names ("xbox") for recognized devices.
function Input:GetGamepadAppearance(device_id)
	local device_name = TheSim:GetGamepadAppearance(device_id)
	-- Do extra processing here to recognize uncommon nonxbox gamepads?
	-- See patterns in https://github.com/idbrii/love-gamepadguesser
	return device_name
end

-- Atlas containing button/key icons for the input image device.
function Input:GetDeviceImageAtlas(device_id)
	assert(device_id ~= KBM_DEVICE_ID, "Keyboard/mouse not handled.")
...
function Input:_SetActiveBinding(control, binding, device_id)
	assert(device_id)
	-- This will never nil deref because we add device tables on connect.
	local active_control = self.active_inputs[device_id][control]

	if active_control
		and active_control[binding]
	then
		-- already had an active binding with the given device_id. Don't process.
		return false
	end

	active_control = self.active_inputs[device_id][control] or {}
	self.active_inputs[device_id][control] = active_control
...
function Input:_PopLastActiveBinding(control, binding, device_id)
	assert(device_id)
	local active_control = self.active_inputs[device_id][control]
	...
	if active_control then
		if active_control[binding] then
			...
			if table.numkeys(active_control) == 0 then
				self.active_inputs[device_id][control] = nil
			end
		end
	end
...
function Input:_SetLastInputDevice(device_type, device_id)
	local old_device_type = self.last_input.device_type
	local changed_device = self.last_input.device_id ~= device_id
...
function Input:OnMouseButtonDownInternal(x, y, button, device_type, is_scroll)
	...
	if bindings then
		for k,v in ipairs(bindings) do
			if self:CheckModifiers(v) then
				if self:_SetActiveBinding(v.control, v, KBM_DEVICE_ID) then
	...
	if not control_set:IsEmpty() then
		self:DoControlDown(control_set, device_type)
		control_set:Clear()
	end
	self.control_set_pool:Recycle(control_set)

	self:_SetLastInputDevice("mouse", "kbm")
end
...
function Input:OnMouseButtonUpInternal(x, y, button, device_type, is_scroll)
	...
	if bindings then
		for k,v in pairs(bindings) do
			if self:CheckModifiers(v) then
				if self:_PopLastActiveBinding(v.control, v, KBM_DEVICE_ID) then
					-- TODO: victorc: Not sure why scrolling doesn't count as a mouse control
					control_set:AddControl(v.control, not is_scroll and "mouse" or "unknown")
				end
			end
	...
	if not control_set:IsEmpty() then
		self:DoControlUp(control_set, device_type)
		control_set:Clear()
	end
	self.control_set_pool:Recycle(control_set)
end
...
-- TODO(input): rename IsModifierCtrl. We use "Control" too often.
...
function Input:OnControlDown(control, device_type, device_id)
	self:LogPrintf("OnControlDown %s %s", device_type, device_id)
	...
	if self.debug == nil or not self.debug:IsConsoleDocked() then
		...
		if device_type == "gamepad" and self.per_gamepad_oncontrol[device_id] then
			self:LogPrintf("OnControlDown HandleEvent %s %s", device_type, device_id)
...
function Input:DoControlDown( control_set, device_type, device_id )
	self:LogPrintf("DoControlDown %s %s", device_type, device_id)
...
function Input:OnControlUp(control, device_type, device_id)
	self:LogPrintf("OnControlUp %s %s", device_type, device_id)
	...
	if self.debug == nil or not self.debug:IsConsoleDocked() then
		...
		if device_type == "gamepad" and self.per_gamepad_oncontrol[device_id] then
			self:LogPrintf("OnControlUp HandleEvent %s %d", device_type, device_id)
...
function Input:OnKeyDown(keyid, modifiers)
	...
	if not control_set:IsEmpty() then
		self:DoControlDown(control_set, "keyboard")
		control_set:Clear()
	end
	self.control_set_pool:Recycle(control_set)
	self:_SetLastInputDevice("keyboard", "kbm")
end
...
function Input:OnKeyUp(keyid, modifiers)
	...
	if self.keys[keyid] then
		...
		if not control_set:IsEmpty() then
			self:DoControlUp(control_set, "keyboard")
			control_set:Clear()
		end
		self.control_set_pool:Recycle(control_set)
	end
...
function Input:OnGamePadButtonDown(gamepad_id, button)
	self:LogPrintf("OnGamePadButtonDown %d %d", gamepad_id, button)
	self.no_input_time = 0
		...
		if bindings then
			for k,v in pairs(bindings) do
				if self:_SetActiveBinding(v.control, v, gamepad_id) then
					...
					self:LogPrintf("OnGamePadButtonDown %d %d binding already active", gamepad_id, button)
				end
			end
		end
	...
	self:_SetLastInputDevice("gamepad", self.gamepads[gamepad_id].name)
end
...
function Input:OnGamePadButtonUp(gamepad_id, button)
	self:LogPrintf("OnGamePadButtonUp %d %d", gamepad_id, button)
	self.no_input_time = 0
		...
		if bindings then
			for k,v in pairs(bindings) do
				if self:_PopLastActiveBinding(v.control, v, gamepad_id) then
					...
					self:LogPrintf("OnGamePadButtonUp %d %d binding already active", gamepad_id, button)
				end
			end
		end
		if not control_set:IsEmpty() then
...
function Input:UpdateRumble(dt)
	...
	for k,rumble_instance in ipairs(self.playing_rumbles) do
		...
		self:LogPrintf("rumble update: device_id=%d rumble_id=%s time=%1.3f",
...
function Input:PlayRumble(device_id, rumble, speed, amp)
	...
	self:LogPrintf("PlayRumble %s device: %d", rumble, device_id)
	...
	for idx, id in ipairs(device_ids) do
		...
		if rumble_data then
			...
			self:LogPrintf("Adding new rumble %s", rumble)
			table.insert(rumbles, rumble)
		else
			self:LogPrintf("Attempt to play invalid rumble: %s", rumble)
		end
	end
...
function Input:OnGamepadConnected(gamepad_id, gamepad_device_description)
	...
	-- Don't use self:LogPrintf so we see device names in bug reports!
...
--		self:LogPrintf("Attempting to assign newly-connected gamepad to player 1...")
--		AllPlayers[1].components.playercontroller:TryChangeInputDevice("gamepad", gamepad_id)
--	end

	for k, v in pairs(self.gamepadconnectedhandlers) do
		v(gamepad_id)
	end
end
...
--	self:LogPrintf("OnGamepadAnalogInput: id=%d ls_x=%1.2f ls_y=%1.2f rs_x=%1.2f rs_y=%1.2f",
...
function Input:DebugListDevices(device_type, dumpAnalogState)
	dumpAnalogState = dumpAnalogState or false
	if device_type == "gamepad" then
		for k,v in pairs(self.gamepads) do
			local device_owner = self.gamepadowners[k]
			local player_id = device_owner and device_owner:GetHunterId()
			TheLog.ch.Input:printf("%s id=%d name=\"%s\" owner=\"%s\" (\"%s\")",
				device_type, k, v.name, player_id and "Player " .. player_id or "free", tostring(device_owner))
			if dumpAnalogState then
	...
	if dumpAnalogState then
		TheInput.enable_logging = not TheInput.enable_logging
		TheLog.ch.Input:printf("Verbose logging %s", TheInput.enable_logging and "ENABLED" or "DISABLED")
	end
end
...
function Input:RegisterGamepadConnectedHandler(inst, fn)
	self.gamepadconnectedhandlers[inst] = fn
end

function Input:UnregisterGamepadConnectedHandler(inst)
	self.gamepadconnectedhandlers[inst] = nil
end
