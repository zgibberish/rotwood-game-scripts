...
function d_playerfollow(follower, leader, behavior)
        TheLog.ch.Cheat:printf("d_playerfollow(<%s>, <%s>, %s)", follower, leader, behavior)
        ...
        local task_name = "attack_" .. follower.GUID
...
function d_startrun(location_id, rng_seed, alt_mapgen_id, ascension)
        TheSaveSystem.cheats:SetValue("skip_new_game_flow", true)
        TheLog.ch.Cheat:print("d_startrun", location_id, rng_seed, alt_mapgen_id)
        location_id = location_id or "kanft_swamp"
        ...
        local start_fn = function()
                ...
                RoomLoader.StartRunWithLocationData(biomes.locations[location_id], rng_seed, alt_mapgen_id, ascension)
        end
...
function d_restartrun()
        if InGamePlay() and TheNet:IsHost() and TheNet:IsInGame() and TheWorld and not TheWorld:HasTag("town") then
                local mode, world, regionID, locationID, seed, altMapGenID, ascension = TheNet:GetRunData()
                if mode == STARTRUNMODE_DEFAULT then
                        d_startrun(locationID, seed, altMapGenID, ascension)
...
function d_draw_world_space_bounds()
        ...
        DEBUG_CACHE.tasks.world_space_bounds = TheWorld:DoPeriodicTask(seconds, function(_)
                ...
                local layout = MapLayout(TheWorld.layout)
                local arena = layout:GetWorldspaceBounds()
