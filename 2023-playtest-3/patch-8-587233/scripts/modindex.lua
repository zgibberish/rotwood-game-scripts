...
local ModIndex = Class(function(self)
	...
	self.savedata =
	{
		known_mods = { },
		known_api_version = 0,
		disable_seasonal_warning = false,
	}
	...
	TheSim:GetPersistentString(filename,
		function(load_success, str)
			if load_success and str == "loading" then
				...
				TheSim:SetPersistentString(filename, self.modsettings.disablemods and "loading" or "done", false, callback)
			end
		end)
end
...
function ModIndex:Save(callback)
	...
	newdata.disable_seasonal_warning = self.savedata.disable_seasonal_warning
    ...
    TheSim:SetPersistentString(self:GetModIndexName(), data, ENCODE_SAVES, callback)
end
...
function ModIndex:LoadModInfo(modname)
	...
	info.version = kstring.trim(info.version or "")
	...
	info.version_compatible = kstring.trim( info.version_compatible )
...
function ModIndex:GetModConfigurationOptions_Internal(modname,force_local_options)
	...
	if known_mod then
		...
		elseif not ModManager.worldgen and (TheNet:IsHost() and not force_local_options) then
...
function ModIndex:SaveConfigurationOptions(callback, modname, configdata, client_config)
    ...
    TheSim:SetPersistentString(name, data, ENCODE_SAVES, cb)
end
...
function ModIndex:SetDisableSpecialEventModWarning()
	self.savedata.disable_seasonal_warning = true
end

function ModIndex:GetIsSpecialEventModWarningDisabled()
	return self.savedata.disable_seasonal_warning
end
...
function ModIndex:Enable(modname)
	...
	if not self.savedata.known_mods[modname].enabled then
		self.savedata.disable_seasonal_warning = false
	end
