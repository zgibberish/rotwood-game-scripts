...
-- TODO(dbriscoe): How does gln/griftlands export their po files? Currently,
-- questral's quest loading relies on TheSim.
local contentloader = require "content.contentloader"
contentloader.ProcessStrings()
...
--print(lkp_var, path)
	...
	local result, val = pcall(function() return load(evalstr)() end)
...
function CreateStringsPOTv2(filename, root, tbl_dta, tbl_lkp)
    ...
    file:write("msgid \"\"\n")
    file:write("msgstr \"\"\n")
	file:write("\"Application: Rotwood\\n\"")
	file:write("\n")
...
-- Don't translate the pretranslated stuff.
STRINGS.PRETRANSLATED = nil
