...
-- KLEI: We don't use luajit, so no ffi.
...
function vec3.new(x, y, z)
	...
	elseif type(x) == "number" then
		dbassert(y == nil, "Pass all three or only one argument.")
		dbassert(z == nil, "Pass all three or only one argument.")
		return new(x, x, x)
	else
		return new()
	end
...
--- Get the unit vector (and optionally length) of a vector.
...
--
-- Common in Rotwood since we often operate on a flat plane. Pass angles from
-- entities as negative radians:
--   Vector3.unit_x:rotate_around_y(-math.rad(inst:GetAngleTo(target)))
--
-- @tparam vec3 a Vector to rotate
-- @tparam number phi Angle to rotate vector by (in radians)
-- @treturn vec3 out
function vec3.rotate_around_y(a, phi)
	-- Copied from vec2.rotate, but using z and returning a Vec3.
	local c = cos(phi)
	local s = sin(phi)
	return new(
		c * a.x - s * a.z,
...
-- No angle_to because that requires defining an axis of rotation. Likely
-- instead, you want to convert both into vec2 and use vec2.angle_to:
--   a:to_xz():angle_to(b:to_xz())
...
local function component_to_string(component)
	return (component and string.format("%+0.3f", component)) or "nil"
end
...
function vec3.to_string(a)
	-- TODO: string.format("(%2.2f, %2.2f, %2.2f)", self.x, self.y, self.z)
	return string.format("(%s, %s, %s)", component_to_string(a.x), component_to_string(a.y), component_to_string(a.z))
end
...
-- KLEI: Use our types in MixedCase style. Naming here only matters for ltags.
local Vector3 = setmetatable({}, vec3_mt)
return Vector3
