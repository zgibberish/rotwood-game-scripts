...
local UnlockableRewardWidget = Class(Widget, function(self, width, owner, level, reward)
	...
	if level or reward then
		self:SetUnlockableData(level, reward)
	end
end)

function UnlockableRewardWidget:SetUnlockableData(level, reward)

	if reward:is_a(MetaProgress.RewardGroup) then

		self.icon = self.icon_root:AddChild( Image(reward.icon) )
			:SetSize(self.icon_width, self.icon_width)
		self.title:SetText(reward.pretty.name)
		self.description:SetText(reward.pretty.desc)
	...
	elseif reward:is_a(MetaProgress.Reward) then
		...
		if def.slot == Consumable.Slots.KEY_ITEMS or def.slot == Consumable.Slots.MATERIALS then
			...
			self.description:SetText(def.pretty.desc)
		else
...
function UnlockableRewardWidget:Layout()
	...
	self.details_bg:SetSize(self.details_w, details_h)
		...
		:Offset(self.icon_width*0.5 + 15, -20)
