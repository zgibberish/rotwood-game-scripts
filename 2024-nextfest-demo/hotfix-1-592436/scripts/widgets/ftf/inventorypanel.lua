...
local InventoryPanel = Class(InventoryBasePanel, function(self, tabs)
	...
	self.listWidget = self:AddChild(InventoryItemList(self.width, self.centerHeight))
		...
		:SetItemTooltipFn(function(itemData, idx) if self.onItemTooltipFn then return self.onItemTooltipFn(itemData, idx) end end)
...
function InventoryPanel:SetOnItemTooltipFn(fn)
	self.onItemTooltipFn = fn
	return self
end
