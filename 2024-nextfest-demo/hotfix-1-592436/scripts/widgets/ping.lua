...
local Ping = Class(Widget, function(self, owner)
    ...
    self.text = self:AddChild(Text(FONTFACE.NUMBERS, 30))
...
function Ping:OnUpdate()
    ...
    if pingVal ~= self.lastPingVal then
        ...
        if self.lastPingVal <= 100 then
            self.text:SetGlyphColour(59/255, 242/255, 99/255, 255/255)
        elseif self.lastPingVal <= 300 then
            self.text:SetGlyphColour(222/255, 222/255, 99/255, 255/255)
        else
