...
BossCoroutine = Class(function(self, inst)
	...
	self.inst:ListenForEvent("dying", function()
		-- If we're dead, disable phasechanged so that we aren't running any coroutines after dying.
		self.phasechanged = nil
		self:Stop("DEAD")
	end)
end)
...
function BossCoroutine:StartNewPhase()
	self.phasechanged = nil
end
...
function BossCoroutine:Start()
	...
	self.practice_run = TheDungeon.progression.components.runmanager:IsPracticeRun()

	if self.practice_run then
		self.health_limit = TheDungeon.progression.components.runmanager:GetBestCommonBossProgress()
	end
