...
function OnChatMessage(senderClientID, senderName, message, isWhisper)
	if not TheWorld
		or not TheDungeon.HUD
	then
		-- TODO(roomtravel): Move chathistory to TheDungeon so we can store messages even if hud/world doesn't exist yet.
		return
	end
	...
	TheDungeon.HUD:ReceiveChatMessage(string.format("<#%s>%s</>: %s", HexToStr(RGBToHex(color)), senderName, message))
