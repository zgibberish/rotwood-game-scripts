...
local function sendsystemmessage(msg)
    if TheDungeon and TheDungeon.HUD then
        TheDungeon.HUD.controls.networkchatqueue:DisplaySystemMessage(msg)
    end
end
...
local function validatevotestart(command, caller, targetid)
    local isdedicated = false-- NW: networking2022: Removed this: not TheNet:GetServerIsClientHosted()
    local clients = nil
    --Check min player count requirements
    if command.voteminpasscount ~= nil then
        clients = TheNet:GetClientTable() -- now called GetClientList, but has a different format
...
local function runcommand(command, params, caller, onserver, confirm)
    ...
    if not onserver and command.confirm and not confirm then
        ...
        if username ~= nil or params.user == nil then
			local title = username ~= nil and string.format(STRINGS.UI.COMMANDSSCREEN.CONFIRMTITLE_TARGET, prettyname(command), username) or string.format(STRINGS.UI.COMMANDSSCREEN.CONFIRMTITLE, prettyname(command))
			local popup = ConfirmDialog()
				:SetTitle(title)
				:SetText(ResolveCommandStringProperty(command, "desc", ""))
				:HideArrow()
				:HideNoButton()
				:CenterButtons()
				:SetYesButton(STRINGS.UI.BUTTONS.OK, function()
					TheFrontEnd:PopScreen() runcommand(command, params, caller, onserver, true)
				end)
...
function GetEmotesWordPredictionDictionary()
	...
	data.GetDisplayString = function(word) return data.prefix .. word end
    return data
end
