...
function Mixer:CreateSnapshot()
    ...
    if top then
        ...
        for k,v in pairs(top.levels) do
            snap:SetLevel(k, TheAudio:GetSoundVolume(k))
        end
        return snap
    end
end
...
function Mixer:GetLevel(lev)
	local val = TheAudio:GetSoundVolume(lev)
    return val
end

function Mixer:SetLevel(name, lev)
    TheAudio:SetSoundVolume(name, lev)
end
...
function Mixer:UpdateFilters(dt)
    ...
    for k,v in pairs(self.lowpassfilters) do
        if v and v.totaltime and v.currenttime and v.totaltime > 0 then
            ...
            if v.currenttime < v.totaltime then
                ...
                TheAudio:SetLowPassFilter(k, v.freq)    
    ...
    for k,v in pairs(self.highpassfilters) do
        if v and v.totaltime and v.currenttime and v.totaltime > 0 then
            ...
            if v.currenttime < v.totaltime then
                v.freq = easing.linear(v.currenttime, v.startfreq, v.endfreq - v.startfreq, v.totaltime)
                TheAudio:SetHighPassFilter(k, v.freq)
            elseif v.currenttime >= v.totaltime and v.freq and v.endfreq and v.freq ~= v.endfreq then
                -- Clamp
                v.freq = v.endfreq
                TheAudio:SetHighPassFilter(k, v.freq)
...
function Mixer:SetLowPassFilter(category, cutoff, timetotake)
	...
	if timetotake <= 0 then
		...
		TheAudio:SetLowPassFilter(category, cutoff)
	end
end
...
function Mixer:SetHighPassFilter(category, cutoff, timetotake)
    ...
    if timetotake <= 0 then
        freq_entry.freq = cutoff
        TheAudio:SetHighPassFilter(category, cutoff)
    end 
end
