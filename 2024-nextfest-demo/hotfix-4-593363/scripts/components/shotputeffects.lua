...
function ShotputEffects:CreateLandingMarker(targetpos)
	if self.landingmarker == nil then
		local playerID = self.inst.owner and self.inst.owner:GetHunterId()
		local prefab = playerID and "fx_ground_target_jamball_p"..playerID or "fx_ground_target_jamball"
		self.landingmarker = SpawnPrefab(prefab, self.inst)
	else
...
function ShotputEffects:OnNetDeserialize()
	...
	if old_last_attack_seqnr ~= self.last_attack_seqnr then
		...
		for i=1,last_attack_target_count do
			...
			if entID then
				...
				TheLog.ch.Shotput:printf("Warning: %s EntityID %d, no entityID for target %d of %d",
					self.inst, self.inst.Network:GetEntityID(), i, last_attack_target_count)
			end
		end
...
function ShotputEffects:OnEntityBecameRemote()
	self.last_attack_seqnr = 0 -- is this needed?
end
