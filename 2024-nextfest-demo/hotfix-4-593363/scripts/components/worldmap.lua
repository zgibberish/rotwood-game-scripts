...
local WorldMap = Class(function(self, inst)
	self.inst = inst -- probably TheDungeon
	self:_Init()
end)

function WorldMap:OnRoomTravel()
	TheLog.ch.WorldMap:print("WorldMap OnRoomTravel")
	self:_Init()
end

function WorldMap:_Init()
	assert(self.inst and self.inst:IsValid())
...
function WorldMap.RoomNavigator:GetProgressForFirstMinibossEncounter()
	-- TODO: If we start using nonguaranteed minibosses, then we need to see if
	-- we're past this progress and whether we actually saw one.
	local biome = self:_GetActiveMapgen()
	return biome.first_miniboss_depth / self.data.max_depth
end

function WorldMap.RoomNavigator:GetRoomForFirstMinibossEncounter()
	-- TODO: If we start using nonguaranteed minibosses, then we need to see if
	-- we're past this progress and whether we actually saw one.
	local biome = self:_GetActiveMapgen()
	return biome.first_miniboss_depth
end
...
-- Get the exits built into the level. They may not have a destination or even
-- be visible. See also HasGateInCardinalDirection.
--
-- return: list of cardinal directions.
function WorldMap:GetCurrentWorldEntrances()
	local prefab = self.inst.room.prefab
	dbassert(prefab:find("_", nil, true), "Unexpected prefab. Should end with _esw or similar.")
	local connect = deconstruct_world_suffix(prefab)
...
local function create_exit_counts(nav, rng, biome)
	...
	local intro_rooms = biome.intro_rooms -- initial rooms that always have 1 exit (incl. entrance)
...
function WorldMap.RoomNavigator:_GetActiveMapgen()
...
function WorldMap:AssignRoomtypesToExits(start_room, additional_depth)
	...
	local biome, biome_location = self.nav:_GetActiveMapgen()
...
local function create_empty_map(size, biome_location, rng)
	...
	if biome_location.monsters
		and biome_location.monsters.minibosses
		and #biome_location.monsters.minibosses > 0
	then
		data.miniboss_prefab = rng:PickValue(biome_location.monsters.minibosses)
	end
...
function WorldMap:LoadMapData(data)
	TheLog.ch.WorldMap:printf("************* WorldMap:LoadMapData called **************")
	if not data then
...
function WorldMap:BuildQuestParams(location_id)
	local biome_location = biomes.locations[location_id]
	assert(biome_location, "Where are we going?")
	local quest = {
		wants_quest_room = false,
	}
	if TheDungeon then
		-- MeetingManager re-evaluates for each room, but worldmap never
		-- changes wants_quest_room. If players drop mid-run, we're okay with
		-- an empty quest room.
		local meetingmanager = TheDungeon.progression.components.meetingmanager
		meetingmanager:EvaluateForNewRun(biome_location)
		quest.wants_quest_room = not not meetingmanager:WantsQuestRoom()
		--~ TheLog.ch.Quest:print("WorldMap evaluating WantsQuestRoom", quest.wants_quest_room)
	else
...
-- A gate goes somewhere, but may be permanently locked. See also
-- GetCurrentWorldEntrances.
function WorldMap:HasGateInCardinalDirection(cardinal)
	...
	if next_room_id and not TheSaveSystem.cheats:GetValue("eliminate_all_exits") then
		return true
	end
...
function WorldMap.RoomNavigator:GetDungeonMiniboss()
	return self.data.miniboss_prefab
end
...
function WorldMap:ShouldSkipMapTransitionFromCurrentRoom()
	if self:IsInBossArea() then
		return true
	end

	local current_room, max_room = self.nav:GetRoomCount_SeenAndMaximum()
	-- these numbers are inflated by 1 because of how the boss room is just added to the end of mapgen
	current_room = current_room - 1
	max_room = max_room - 1
	local miniboss_room = self.nav:GetRoomForFirstMinibossEncounter()
	local room_before_hype = max_room - 1

	return not (current_room == miniboss_room or current_room == room_before_hype)
end
...
function WorldMap:DebugDrawEntity(ui, panel, colors)
	if ui:CollapsingHeader("Room Explorer", ui.TreeNodeFlags.DefaultOpen) then
		...
		ui:Spacing()
		if ui:CollapsingHeader("Limits", ui.TreeNodeFlags.DefaultOpen) and self.data.gen_state.dbg_tracker then
			ui:Indent()
			local rec = self.data.gen_state.dbg_tracker[debug_rid]
			if rec then
				ui:TextColored(colors.header, "Data used when assigning our roomtype")
				ui:Value("snooze_seen", table.inspect(rec.hist.snooze_seen))
				ui:Value("snooze_depth", table.inspect(rec.hist.snooze_depth))
				ui:Value("seen", table.inspect(rec.hist.seen))
				ui:Value("visited", table.inspect(rec.hist.visited))
			end
			ui:Unindent()
		end
...
local function mock_CreateMap()
	...
	local map = WorldMap(mock.dungeon())
