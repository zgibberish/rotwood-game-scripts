...
for _,shake_set in pairs(ShakeDeltas) do
	for i,delta in ipairs(shake_set) do
		shake_set[i] = delta:normalized()
	end
end

local CameraShake = Class(function(self, mode, duration, speed, scale)
	self:StopShaking() -- init values
...
function CameraShake:Update(dt)
	...
	local shakeDeltas = ShakeDeltas[self.mode] or ShakeDeltas[CAMERASHAKE.FULL]
