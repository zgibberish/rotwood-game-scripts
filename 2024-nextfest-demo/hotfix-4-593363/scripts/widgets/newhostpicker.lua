...
local NewHostPicker = Class(Widget, function(self)
    ...
    for i, v in ipairs(newhost_options) do
        self.buttons[i] = self:AddChild(ImageButton("images/ui_dst.xml", "in-window_button_tile_idle.tex", "in-window_button_tile_hl.tex", "in-window_button_tile_disabled.tex", "in-window_button_tile_hl_noshadow.tex", "in-window_button_tile_disabled.tex", tile_scale, { 0, 0 }))
        ...
        self.buttons[i]:SetFont(FONTFACE.BUTTON)
    ...
    self.headertext = self:AddChild(Text(FONTFACE.BUTTON, FONTSIZE.BUTTON, STRINGS.UI.SERVERCREATIONSCREEN.NEWHOST_TITLE))
    self.headertext:SetRegionSize(800, 40)
    self.headertext:SetPosition(0, -25)
    self.headertext:SetVAlign(ANCHOR_TOP)
    self.headertext:SetGlyphColour(0,0,0,1)

    self.description = self:AddChild(Text(FONTFACE.DEFAULT, 30, ""))
    self.description:SetRegionSize(510, 280)
    self.description:SetPosition(0, -400)
    self.description:SetGlyphColour(0,0,0,1)
