		...
		Updater.Do(function()
			...
			self.dungeon_level:ShowMetaProgression(function(level_num, move_up, reward_earned, next_reward, sequence_done)
				...
				if reward_earned then
					self.reward_container:Show():AddReward(reward_earned, level_num, true)
					self:PlaySpatialSound(fmodtable.Event.ui_endOfRun_unlock_power)
				end

				-- Called to show the upcoming reward
				if next_reward then
					self.reward_container:Show():AddReward(next_reward, level_num, false)
					self:PlaySpatialSound(fmodtable.Event.ui_endOfRun_unlock_power)
				end
...
function PlayerDungeonSummary:DebugDraw_AddSection(ui, panel)
	PlayerDungeonSummary._base.DebugDraw_AddSection(self, ui, panel)
