...
function Grid:Clear()
	for y,col in pairs(self.items_by_coords) do
		for x,v in pairs(col) do
			v:Kill()
		end
	end
    self.items_by_coords = {}
...
--      local g = Grid()
--      g:FillGrid(2, 100, 100, { Text(FONTFACE.DEFAULT, 20, "1"), Text(FONTFACE.DEFAULT, 20, "2"), Text(FONTFACE.DEFAULT, 20, "3"), })
-- Gives a grid like:
--      1  2
--      3
...
function Grid:DebugDraw_AddSection(ui, panel)
    Grid._base.DebugDraw_AddSection(self, ui, panel)

    ui:Spacing()
    ui:Text("Grid")
    ui:Indent() do
        ui:Value("max rows", self.rows)
        ui:Value("max cols", self.cols)

        local changed_layout, should_layout = ui:Checkbox("use natural layout", self.layout_left_to_right_top_to_bottom)
        if changed_layout then
            self.layout_left_to_right_top_to_bottom = should_layout
        end

        local changed_h, new_h = ui:DragFloat("h_offset", self.h_offset, 10, 0, 10000, "%.1f", 1)
        local changed_v, new_v = ui:DragFloat("v_offset", self.v_offset, 10, 0, 10000, "%.1f", 1)
        if changed_h then
            self.h_offset = new_h
        end
        if changed_v then
