...
local CountdownBeta = Class(Widget, function(self, owner, mode, image, update_name, release_date)
	...
	if mode == "text" then
		...
		self.daysuntiltext = self:AddChild(Text(FONTFACE.NUMBERS, FONTSIZE.SCREEN_SUBTITLE))
	...
	elseif mode == "image" or mode == "reveal" or mode == "released" then
		...
		self.title = lableroot:AddChild(Text(FONTFACE.NUMBERS, FONTSIZE.SCREEN_SUBTITLE))
		...
		self.daysuntiltext = lableroot:AddChild(Text(FONTFACE.NUMBERS, 30))
		...
		if mode == "reveal" then
			self.title2 = lableroot:AddChild(Text(FONTFACE.NUMBERS, 25))
			...
			self.reveal_image:SetMultColor(1,1,1,0)
	...
	elseif mode == "quagmire" then
		...
		self.button = lableroot:AddChild(ActionButton())
			:SetName("Button")
			:SetSize(BUTTON_W, BUTTON_H)
			:SetPrimary()
			:SetText(STRINGS.UI.MODSSCREEN.MODLINK_MOREINFO)
			:SetOnClick(function() VisitURL(URLS.coming_soon) end)
