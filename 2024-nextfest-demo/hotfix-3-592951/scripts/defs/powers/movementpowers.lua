	...
	on_update_fn = function(pow, inst, dt)
		...
		for _, source in pairs(pow.mem.sources) do
			if CheckPushSourceValid(pow, inst, source) then
				...
				local attack_type = source.sg.statemem.attack_type or "SUPER_FLAP"
				local pushspeed = 0
				local tuning = pow.persistdata:GetVar(attack_type)

				if attack_type == "SUPER_FLAP" then
				...
				inst.components.pushforce:AddPushForce(pow.def.name, source, source_dir * pushspeed * dist_mult)
			end
		end
	end,
