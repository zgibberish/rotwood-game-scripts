...
local function UpdateKonjurState(inst)
	if inst.components.battoadsync:GetStolenKonjur() > 0 then
...
local function StealKonjur(inst, target)
	if target.components.inventoryhoard then
		...
		if konjur > 0 then
			...
			inst.components.battoadsync:SetStolenKonjur(steal_amount)
...
local function ConsumeKonjur(inst)
	...
	inst.components.battoadsync:SetStolenKonjur(0)
	UpdateKonjurState(inst)
end

local function DropKonjur(inst)
	if inst.components.battoadsync:GetStolenKonjur() > 0 then
		local loot = { ["konjur"] = inst.components.battoadsync:GetStolenKonjur() }
		LootEvents.SpawnLootFromMaterials(inst, loot)

		TheDungeon.HUD:MakePopText({ target = inst, button = string.format(STRINGS.UI.INVENTORYSCREEN.KONJUR, inst.components.battoadsync:GetStolenKonjur()), color = UICOLORS.KONJUR, size = 35, fade_time = 3 })
		inst.components.battoadsync:SetStolenKonjur(0)
