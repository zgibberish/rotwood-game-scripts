...
describe("color:", function()
	it("operators: add, subract, multiply", function()
		...
		assert_is_float_equal(r[4], 5)
		assert_is_float_equal(r.r, 5)
		assert_is_float_equal(r.g, 5)
		assert_is_float_equal(r.b, 5)
		assert_is_float_equal(r.a, 5)
	end)

	it("rgb -> hsv -> rgb", function()
		local c = color(1,1,1,1)
		local hsv = c:color_to_hsb_table()
		local c1 = color.hsb_to_color_table(hsv)
		local c2 = color.from_hsba(hsv[1], hsv[2], hsv[3], hsv[4])
		local c3 = color.from_hsb(hsv[1], hsv[2], hsv[3])
		c3[4] = c[4]
		for i=1,4 do
	...
	it("hsv -> rgb -> hsv", function()
		...
		for h=0,1, 0.1 do
			hsv1[1] = h
			local cc = color.hsb_to_color_table(hsv1)
			local hsv2 = cc:color_to_hsb_table()
			for i=1,4 do
	...
	it("set hsv", function()
		...
		assert_is_approx_equal(hsv.h, 300/360)
		assert_is_approx_equal(hsv.s, 0.8361)
		assert_is_approx_equal(hsv.b, 0.4784)
