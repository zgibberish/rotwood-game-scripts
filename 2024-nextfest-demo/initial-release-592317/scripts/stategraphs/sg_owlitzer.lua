...
local function SetWindAttackType(inst, type)
	inst.sg.statemem.attack_type = type
end
...
local states =
{
	...
	State({
		...
		timeline =
		{
			...
			FrameEvent(4, function(inst)
				SetWindAttackType(inst, "WIND_GUST")
	...
	State({
		...
		onenter = function(inst)
			...
			local pos = inst:GetPosition()
			local spawn_pos = spawnutil.GetStartPointFromWorld(inst.Transform:GetFacing() == FACING_LEFT and 0.75 or 0.25, .05)
...
SGCommon.States.AddAttackHold(states, "dive_bomb",
{
	...
	onenter_fn = function(inst)
		...
		inst.sg.statemem.warning_ticks = 75 * 2 -- make sure this equals the FrameEvent in timeline that locks in the target
		--sound
		local params = {}
		params.fmodevent = fmodtable.Event.owlitzer_targetLock_LP
		params.sound_max_count = 1
	...
	timeline =
	{
		-- startup frames (120) - pre anim frames (48) - time before attack to show target FX (0.15s -> 9 frames)
