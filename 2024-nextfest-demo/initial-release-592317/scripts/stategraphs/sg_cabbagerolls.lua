	...
	State({
		...
		timeline =
		{
			...
			FrameEvent(9, function(inst)
				...
				local length = inst:HasTag("elite") and 6.8 or 6.1
			...
			FrameEvent(11, function(inst)
				local length = inst:HasTag("elite") and 7.1 or 6.5
				inst.components.hitbox:PushBeam(1, length, 1, HitPriority.MOB_DEFAULT)
			end),
			...
			FrameEvent(21, function(inst)
				...
				inst.components.hitbox:StopRepeatTargetDelay()
				inst.components.hitbox:StartRepeatTargetDelay()
				local length = inst:HasTag("elite") and -6.3 or -5.9
			...
			FrameEvent(23, function(inst)
				local length = inst:HasTag("elite") and -6.5 or -6.1
				inst.components.hitbox:PushBeam(0, length, 1, HitPriority.MOB_DEFAULT)
			end),
