...
function Menu:AddItem(text, cb, offset, style, size, control)
    ...
    local button
    if style == "small" then
        button = self:AddChild(ImageButton("images/ui_dst/button_small.tex", "images/ui_dst/button_small_over.tex", "images/ui_dst/button_small_disabled.tex", nil, nil, {1,1}, {0,0}))
        button.image:SetScale(1.1)
        button.text:SetPosition(2,-2)
        button:SetFont(FONTFACE.BUTTON)
        button:SetTextColour(HexToRGB(0x000000ff))
    elseif style == "tabs" then
        ...
        button = self:AddChild(ImageButton(
                "images/frontend_redux/list_tabs_normal.tex",
                nil,
                nil, -- "list_tabs_disabled.tex",
                nil, -- "list_tabs_down.tex",
                "images/frontend_redux/list_tabs_selected.tex",
                {scale},
                {0,4}
            ))
        ...
        button:SetFont(FONTFACE.BUTTON)
        button:SetDisabledFont(FONTFACE.BUTTON)
        button:SetTextColour(UICOLORS.GOLD_CLICKABLE)
        button:SetTextFocusColour(UICOLORS.GOLD_FOCUS)
        button:SetTextDisabledColour(UICOLORS.GOLD_UNIMPORTANT)
        button:SetTextSelectedColour(UICOLORS.BLACK)
    elseif style == "none" then
        button = self:AddChild(Button())
        button:SetFont(FONTFACE.BUTTON)
        button:SetTextColour(HexToRGB(0x000000ff))
    else
        local is_odd = self.children and #self.children % 2 == 0
        button = self:AddChild(PanelButton(is_odd and "images/ui_ftf/error_btn_odd.tex" or "images/ui_ftf/error_btn_even.tex"))
            :SetNineSliceCoords(6, 0, 344, 90)
            :SetSize(self.offset - 15, 90)
            :SetImageNormalColour(HexToRGB(0xFFF1E2ff))
            :SetImageFocusColour(HexToRGB(0xFFCB27ff))
            :SetTextColour(HexToRGB(0x755751ff))
            :SetTextFocusColour(HexToRGB(0x000000ff))

        if style == "NEGATIVE_BUTTON_STYLE" then
            button:SetImageNormalColour(HexToRGB(0xEF5D5Dff))
                :SetImageFocusColour(HexToRGB(0xFFCB27ff))
                :SetTextColour(HexToRGB(0x6C0E0Eff))
                :SetTextFocusColour(HexToRGB(0x000000ff))
        end
    end
    ...
    if control ~= nil then
        button.prompt_shadow = button:AddChild(Text(FONTFACE.DEFAULT, size))
        button.prompt = button:AddChild(Text(FONTFACE.DEFAULT, size))
        local str = TheInput:GetLabelForControl(control).." "..text
        button.prompt:SetString(str)
        button.prompt_shadow:SetString(str)
        button.prompt_shadow:SetGlyphColor(WEBCOLORS.BLACK)
    else
        button:SetText(text)
    end
...
function Menu:AutoSpaceByText(spacing)
    ...
    for i, v in ipairs(self.items) do
        local w, h = 0, 0
        if #v.text:GetText() > 0 then
            w, h = v.text:GetRegionSize()
        end
        if v.prompt ~= nil and #v.prompt:GetText() > 0 then
