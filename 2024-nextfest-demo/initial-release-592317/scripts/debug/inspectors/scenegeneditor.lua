...
function SceneGenEditor:GetZoneGrid()
	if not self.cached_zone_grid then
		self.cached_zone_grid = TheWorld and ZoneGrid(TheWorld.map_layout)
	end
...
function SceneGenEditor:GetFx()
	local zone_gen = self:GetZoneGen()
	return (zone_gen and self.selected_fx)
		and zone_gen.fxes[self.selected_fx]
		or nil
end
...
function SceneGenEditor:BrowserElementUi(ui, id, browser_element)
	...
	then
		...
		return true -- Yes, we put a button on the line.
	elseif self.browser.context == PrefabBrowserContext.id.Fx then
		local zone_gen = self:GetZoneGen()
		if not zone_gen then
			return false
		end
		...
		if ui:Button(ui.icon.add .. id) then
			if not zone_gen.fxes then
				zone_gen.fxes = {}
			end
			local added = SceneFx(browser_element, self:GetFx())
			table.insert(zone_gen.fxes, added)
			table.insert(self.load_queue, browser_element)
			self.dirty_stuff.fxes = true
		end
		ui:SetTooltipIfHovered("Add fx to ZoneGen")

		local fx = self:GetFx()
		if fx and fx.fx ~= browser_element then
			ui:SameLineWithSpace()
			if ui:Button(ui.icon.undo .. id) then
				fx.fx = browser_element
				self.dirty_stuff.fxes = true
			end
			ui:SetTooltipIfHovered("Replace fx with this one")
		end
...
function SceneGenEditor:CollectZoneDependencies(zone_gens)
	...
	for _, zone_gen in ipairs(zone_gens) do
		local decor_layer = zone_gen.decor_layer or DecorLayer.s.Ground
		zones[decor_layer] = table.appendarrays(zones[decor_layer], zone_gen.zones)
	end
