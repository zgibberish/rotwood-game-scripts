...
local PlayerHud = Class(Screen, function(self)
	...
	if TheDungeon:IsInTown() then
		...
		TheWorld.components.chathistory:Clear()
	...
	TheWorld.components.chathistory:Load()
...
function PlayerHud:ReceiveChatMessage(msg)
	TheWorld.components.chathistory:Append(msg)

	self:RefreshChat()
end

function PlayerHud:ShowChatHistory()
	local history = TheWorld.components.chathistory:GetHistory()
...
function PlayerHud:StartDungeonTravelPreview(cardinal)
	...
	local current_room, max_room = TheDungeon:GetDungeonMap().nav:GetRoomCount_SeenAndMaximum()

	-- these numbers are inflated by 1 because of how the boss room is just added to the end of mapgen
	current_room = current_room - 1
	max_room = max_room - 1

	local miniboss_room = TheDungeon:GetDungeonMap().nav:GetRoomForFirstMinibossEncounter()
	local room_before_hype = max_room - 1

	
	if current_room == miniboss_room -- If you're leaving the miniboss room
		or current_room == room_before_hype then -- if you're entering the hype room
			self.travel = TravelScreen(cardinal)
			TheFrontEnd:PushScreen(self.travel)
	else
		TheFrontEnd:Fade(FADE_OUT, 1, function()
			if TheNet:IsHost() then
				TheLog.ch.TravelScreen:printf("Host traveling %s ...", cardinal)
				TheDungeon:GetDungeonMap():TravelCardinalDirection(cardinal)
			else
				TheLog.ch.TravelScreen:printf("Client waiting for host to travel %s ...", cardinal)
			end
		end)
	end
