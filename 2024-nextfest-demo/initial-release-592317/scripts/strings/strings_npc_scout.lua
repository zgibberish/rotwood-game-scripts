return
{
	QUIPS =
	{
		quip_scout_generic =
		{
			[[
				!gesture
				Just hop in the <#RED>{name.damselfly}</> when you're ready to Hunt.
			]],
			[[
				!gesture
			...
			[[
				!gesture
				No pressure, but lemme know when you're ready. I'm itching to get back in the air!
			]],
			[[
				!point
				Remember to stay safe and prioritize <#RED>Dodging</> over <#RED>{name.concept_damage}</> out there.
			]],
			[[
				!gesture
			...
			[[
				!gesture
				Don't forget, you can practice your moves on the training dummies around here.
			]],
			[[
				!gesture
				Ready to {name.run}?
		...
		quip_scout_has_armoursmith = {
			[[
				!gesture
				It's nice to have <#BLUE>{name.npc_armorsmith}</> around. I think she brightens up the camp.
			]],
			[[
				!gesture
				Feel free to chat with folks around town. It's good for morale.
			]],
			[[
				!gesture
				Take some time to socialize with the other {name_multiple.foxtails}. It's good for team cohesion.
			]],
		},
		quip_scout_no_armoursmith = {
			[[
				!dejected
		...
		quip_scout_has_blacksmith = {
			[[
				!gesture
				I'm glad I hired <#BLUE>{name.npc_blacksmith}</>. I can trust him to be honest with me.
			]],
		},
		quip_scout_no_blacksmith = {
			[[
				!dejected
			...
			[[
				!nervous
				I think I saw some of <#BLUE>{name.npc_blacksmith}'s</> tools from the {name.damselfly}... but no <#BLUE>{name.npc_blacksmith}</>.
			]],
			[[
				!nervous
		...
		quip_scout_no_dojo = {
			[[
				!thinking
				Hmm... I thought <#BLUE>{name.npc_dojo_master}</> would be back by now.
			]],
			[[
				!shrug
				I wonder what's taking <#BLUE>{name.npc_dojo_master}</> so long to get back.
			]],
			[[
				!thinking
				I hope <#BLUE>{name.npc_dojo_master}</> hasn't run into any trouble.
			]],
			[[
				!thinking
				<#BLUE>{name.npc_dojo_master}</> will be back soon. I'm sure of it.
			]],
			[[
				!nervous
				I didn't overestimate <#BLUE>{name.npc_dojo_master}'s</> battle readiness did I?
				!think
				I hope he's okay.
			]],
			[[
				!think
				Hopefully <#BLUE>{name.npc_dojo_master}'s</> back soon.
				!gesture
				Our Hunters need a {name.job_dojo}.
			]],
			[[
				!think
				I hope <#BLUE>{name.npc_dojo_master}</> isn't having trouble out there.
				!nervous
		...
		quip_scout_has_cook = {
			...
			[[
				!gesture
				Don't forget, you'll get some good <#RED>Buffs</> from <#BLUE>{name.npc_cook}'s</> cooking.
			]],
		},
		quip_won_last_run = {
			[[
				!think
				I didn't think to pack bug spray for this expedition. I regret that.
			]],
			[[
				!closedeyes
			...
			[[
				!gesture
				There are few things more refreshing that the wind rushing through my fur.
			]],
			[[
				!laugh
				I bet you enjoy the flight home more when you're conscious, huh? Or, remember more of the scenery at least.
			]],
		},
		quip_lost_last_run = {
			[[
			...
			[[
				!greet
				Hey, Hunter. No lasting damage from that last Hunt, I hope.
			]],
		},
		quip_abandoned_last_run = {
			[[
				!gesture
		...
		dojo_inhalerquest_one = {
			...
			mid_quest_quips = {
				[[
					!gesture
					Let's get back out there. I wanna get <#BLUE>{name.npc_dojo_master}</> his inhaler.
				]],
				[[
					!gesture
					Let's not waste too much time. <#BLUE>{name.npc_dojo_master}'s</> in bad shape.
				]],
			},
		},
	...
	{	
		main_defeat_megatreemon =
		{
			...
			die_to_miniboss_convo =
			{
				...
				TALK2 = [[
					agent:
						!point
						Uh yeah, you got your butt kicked <i>by a <#RED>{miniboss}</>!</i>
						!think
						Bigger <#RED>{name_multiple.rot}</> tend to hang out around <#RED>{name_multiple.rot_boss}</>, y'know.
						!point
						If you've encountered a <#RED>{miniboss}</>, that must mean you're getting closer to the <#RED>{name.megatreemon}</>!
				]],
			...
			talk_after_konjur_heart =
			{
				...
				TALK2 = [[
					agent:
						!point
						When you put a <#KONJUR>{name.konjur_heart}</> in the <#KONJUR>{name.town_grid_cryst}</> it works kinda like a prism, amplifying one aspect of your Hunter abilities.
						!gesture
						It looks like <#RED>{name.megatreemon}'s</> <#KONJUR>{name.konjur_heart}</> gave you some extra <#RED>Health</>. Can't complain about that!
				]],
		...
		main_defeat_owlitzer =
		{
			...
			quest_intro =
			{
				...
				OPT_1A = "I can actually see the stars!",
				OPT_1B = "<i>Brr</i>. Feels a little breezy.",
				OPT_1C = "What can you tell me about the {name.rot_boss}?",
				...
				OPT1A_RESPONSE = [[
					agent:
						...
						Isn't it nice?
						!think
						I think the wind blows away some of the <#KONJUR>{name.konjur}</> smog that hangs in the air everywhere else.
				]],
				OPT1B_RESPONSE = [[
					agent:
						!agree
				...
				OPT2_RESPONSE = [[
					agent:
						!agree
						Mhm. Some of it's natural, but <#RED>{name.owlitzer_forest}</> is also full of <#RED>{name_multiple.rot}</> with <#RED>Wind</> powers.
						!point
						If you've got <#RED>{name.heavy_weight} Class</> gear the <#RED>Wind</> won't bother you so much, but any lighter and you might find it hard to keep your footing.
				]],

				OPT_3 = "Can you still fly over me if it's windy?",

				OPT3_RESPONSE = [[
					agent:
						!laugh
						Oh, yeah. The {name.damselfly} and I took a direct hit from a <#RED>{name.megatreemon}</>, remember.
						!gesture
						We can handle a stiff breeze.
				]],
				--END BRANCH--
				...
				OPT1C_RESPONSE = [[
					agent:
						!point
						The <#RED>{name.owlitzer}</> that roosts at the center of this grove has our next <#KONJUR>{name.konjur_heart}</>.
						!think
				...
				OPT_4A = "<i>Perfect!</i> I love a challenge.",
				OPT_4B = "You're not inspiring a lot of confidence.",

				OPT4A_RESPONSE = [[
					agent:
						!gesture
						I don't know where you get your bravery, Hunter.
						!clap
						But I like it!
						!point
						Let's head out!
			...
			celebrate_defeat_boss =
			{
				...
				TALK_FIRST_BOSS_KILL = [[
					agent:
						!gesture
						Hey Hunter, I hope you've been enjoying your time here.
						!agree
						And that you'll visit us again for <#RED>Early Access</>.
						!shocked
						I really can't believe you've taken down <i>two</i> unique <#RED>{name_multiple.rot_boss}</> since we met!
						!shocked
						Now the real fun begins!
						!gesture
						You have a whole gamut of <#RED>Frenzy Levels</> to beat, y'know!
						!dubious
						Ready to head out?
				]],

				OPT_1 = "Let's fly, Flitt!",
				...
				TALK2 = [[
					agent:
						...
						Oh, yeah!
						!laugh
						Don't worry, he's visiting family back in the Brinks. He'll be back later.
						!nervous
			...
			quest_intro =
			{
				...
				OPT1B_RESPONSE = [[
					agent:
						...
						!gesture
						Ten years of <#KONJUR>{name.i_konjur}</> contamination's made the <#RED>Acid</> and <#RED>Spores</> waaay more potent.
				]],
			...
			foraging = {
				part_one = {
					TALK = [[
						agent:
							!dejected
							I was thinking about doing some foraging in the woods... but everything out there's probably too <#KONJUR>{name.i_konjur}</>-contaminated for me to eat.
							!shocked
							Oh!
							!bliss
							But I guess I could just give whatever I find to you Hunters!
					]],
		...
		twn_friendlychat = 
		{
			...
			DOJO_QUESTION2 = "How do you and {name.npc_dojo_master} know each other?",
			DOJO_TALK2 = {
				TALK = [[
					agent:
						!laugh
				...
				TALK2 = [[
					agent:
						...
						!shrug
						I might not be such a good {name.job_scout} today if not for all that time I spent sneaking around. 
						!point
				...
				TALK4 = [[
					agent:
						!bliss
						Haha! He got a ton of signups that day!
				]],
				...
				OPT4_RESPONSE = [[
					agent:
						!disagree
						No. He squinted his eyes and clocked me instantly.
						!gesture
				...
				OPT5A_RESPONSE = [[
					agent:
						...
						Yup!
						!gesture
						No one can be their best selves when they're stuck in survival mode.
						!point
				...
				OPT5B_RESPONSE = [[
					agent:
						...
						Absolutely!
						!think
						Although I'm not sure he'd use that word to describe himself.
						!point
						The fact he came on this expedition tells me all I need to know.
				]],
			...
			KONJUR_TECH_KNOWLEDGE_QUESTION = "How do you know so much about the {name.i_konjur} tech around here?",
