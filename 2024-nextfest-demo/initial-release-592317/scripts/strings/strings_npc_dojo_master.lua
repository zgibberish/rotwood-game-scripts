		...
		twn_shop_dojo =
		{
			DODGE_CONVERSATION = {
				...
				TALK2 = [[
					agent:
						!point
						If your just charge into a Hunt <#RED>Weapons</> swingin', you'll get knocked on yer tail in no time flat.
						!dubious
						If you wanna stand a chance, slow down and take in your surroundings.
						!gesture
						Watch your foes' movement for clues they're about to strike.
						!point
						Then <#RED>{name.concept_dodge}</> out of the way with <p bind='Controls.Digital.DODGE' color=BTNICON_DARK> when they do.
						!gruffnod
						Often foes leave themselves wide open after they <#RED>{name.concept_attack}</>, but you won't be able to take advantage of it if you're reelin' from a hit.
				]],
			...
			FOCUS_HIT_CONVERSATION = {
				...
				OPT2A_RESPONSE = [[
					agent:
						...
						!gesture
						Plus, unlike <#ATK_CRIT>{name_multiple.concept_critical}</>, you can rely on them to happen at will.
						!point
						So learn how to do'em if you don't wanna deal subpar <#RED>{name.concept_damage}</>!
						!gruffnod
				...
				OPT2B_RESPONSE = [[
					agent:
						...
						!point
						<#BLUE>{name_multiple.concept_focus_hit}</> are <#RED>{name_multiple.concept_attack}</> that deal way, <i>way</i> more <#RED>{name.concept_damage}</> than regular <#RED>{name_multiple.concept_attack}</>.
						!gesture
						And unlike <#ATK_CRIT>{name_multiple.concept_critical}</>, you can rely on them to happen at will... once you've learned your <#RED>Weapon's</> <#BLUE>{name.concept_focus_hit}</> conditions.
						!gruffnod
						Don't know your <#RED>Weapon's</> conditions? Swing it here in town and a tip'll pop up to remind you!
						!shrug
				...
				OPT2C_RESPONSE = [[
					agent:
						!shocked
						CORRECT! <#BLUE>{name_multiple.concept_focus_hit}</> do waaay more <#RED>{name.concept_damage}</> than normal <#RED>{name_multiple.concept_attack}</>!
						!gruffnod
						Making them a regular part of your strategy will boost your <#RED>{name.concept_damage}</> output and give you a major leg up on the battlefield.
						!dubious
						But they only happen when you fulfill your <#RED>Weapon's</> <#BLUE>{name.concept_focus_hit}</> conditions.
						!gesture
						Don't know the conditions for you <#RED>Weapon</>? Swing it here in town and a tip'll pop up to remind you!
						!shrug
						Just don't hit no one.
				]],
				...
				OPT2D_RESPONSE = [[
					agent:
						...
						!gesture
						<#BLUE>{name_multiple.concept_focus_hit}</> do significantly more <#RED>{name.concept_damage}</> than regular <#RED>{name_multiple.concept_attack}</>.
						!point
						And unlike <#ATK_CRIT>{name_multiple.concept_critical}</>, you can make <#BLUE>{name_multiple.concept_focus_hit}</> happen at will.
						!shrug
						If you change your mind, you can swing your <#RED>Weapon</> anywhere in town to pop up a tip showing its <#BLUE>{name.concept_focus_hit}</> conditions.
						!gruffnod
						You'll know you performed one right if your <#RED>Damage</> numbers appear in <#BLUE>Blue</>.
				]],
				...
				TUTORIALS = {
					...
					GENERAL = {
						...
						FRENZIED_HUNTS = [[
							agent:
								!point
								<#RED>Frenzied</> Hunts are tougher than regular Hunts, but you need them for progression-- they're the only way to get those precious <#KONJUR>{name_multiple.boss_heart}</>.
								!dubious
