...
function RootAttacker:SetSyncCommand(command, data)
	if self:CanUpdateSyncedData() then
		...
		if self.is_command_queued then
			TheLog.ch.RootAttacker:printf("Warning: Stomping command %s with %s before it was serialized!",
				self.command == RootCommandIdle and "Idle" or RootCommand:FromId(self.command),
				command == RootCommandIdle and "Idle" or RootCommand:FromId(command))
		end
		self.command = command
		self.command_data = data
		self.is_command_queued = true
...
function RootAttacker:OnNetDeserialize()
	...
	if new_command ~= self.command then
		if self.command ~= 0 then
			...
			-- failsafe for despawning guard roots on clients
			if new_command ~= RootCommand.id.DespawnGuardRoots and #self.guard_roots > 0 then
				TheLog.ch.RootAttacker:printf("Warning: Received new command = %s before despawning guard roots.  Despawning now.",
					RootCommand:FromId(new_command))
...
function RootAttacker:IsIdle()
	return self.command == RootCommandIdle
end
