...
function ForcedLocomote:IsForcingMove()
	return not not self.move_task
end
...
function ForcedLocomote:ChaseEntity(target, threshold)
	if not self.inst:IsLocal() then
		TheLog.ch.ForcedLocomote:printf("ChaseEntity called on non-local entity <%s>! Aborting.", target)
		return
	end
	--~ TheLog.ch.ForcedLocomote:printf("ChaseEntity(<%s>, %s)", target, threshold)
	self:AbortMove()
